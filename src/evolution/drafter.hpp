// 自进化闭环阶段 2/5/6:EvolutionDrafter 的纯函数芯。
...
//   阶段 6:簇内多场同求一件不存在的工具(§3.5 新执行能力) -> process
//           Plugin 草稿(plugin.json + runner 脚手架 + 依赖清单);草稿
//           永不执行、永不挂载,补实现交人工(见下方"阶段 6"节)。
//
...
struct ComboCandidateDraft {
    ...
    // 代码档(阶段 6):process Plugin 草稿。with_plugin_draft 置真时
    // workflow/agent 一律不添——插件还没真身,编排等它落地后的下一只候选;
    // 草稿永不进挂载事务、不自动启用(零进程铁律)。
    bool with_plugin_draft = false;
    std::string plugin_id;            // plugins/<id>/ 的 <id>(小写 kebab)
...
// 观察账上"想用而不可得"的失败码(agent 循环对未注册工具的实报)。
inline constexpr const char* kUnknownToolErrorCode = "registry.unknown_tool";
