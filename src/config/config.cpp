...
// 鉴权 setter 三件(骨架拆解反弹·问题 4):改法与落盘侧
// SetProviderXxxInGlobalConfig 的 mutate 逐字对齐;空 key/空变量名是半截
// 配置,拒绝不落。语义与用法见 config.hpp 的声明处注释。
...
bool SetProviderAuthNone(std::vector<ProviderConfig>& providers, const std::string& name) {
    for (ProviderConfig& provider : providers) {
        if (provider.name == name) {
            provider.auth = ProviderAuthMode::None;
