...
//   2. 本件的 raw 扫——fold 不留的字段按 envelope 逐行取(turn 归属、
//      training_policy、is_error、purpose、outcome.assessed payload、
//      context.attached、环境快照档位、证据新鲜度对照线);
...
std::string GetString(const nlohmann::json& value, const char* key) {
    const auto it = value.find(key);
    return it != value.end() && it->is_string() ? it->get<std::string>() : std::string();
}

bool GetBool(const nlohmann::json& value, const char* key) {
    const auto it = value.find(key);
    return it != value.end() && it->is_boolean() ? it->get<bool>() : false;
}

std::uint64_t GetUint(const nlohmann::json& value, const char* key) {
    const auto it = value.find(key);
    if (it == value.end()) {
        return 0;
    }
...
// §9.2 四档 -> §11.4 replayability 轴值。
std::string MapReplayLevelToAbility(const std::string& level) {
    if (level == "exact") {
...
// ---------------------------------------------------------------------------
// 隐私扫描(§十二:exporter 再扫一遍;命中进 excluded,报 source event id)
// ---------------------------------------------------------------------------
...
// 与 "https://x" 都不算绝对路径。
bool AtTokenBoundary(std::string_view text, std::size_t at) {
    if (at == 0) {
        return true;
    }
...
bool StartsWithCaseInsensitive(std::string_view text, std::size_t at, std::string_view needle) {
...
//      /workspace——系统绝对路径。
void ScanTextForPaths(std::string_view text, const std::string& event_id,
...
bool LooksLikeBinary(std::string_view text) {
...
// redactor 同款先例)。
void ScanTextForPrivacy(std::string_view text, const std::string& event_id,
                        std::vector<PrivacyFinding>* findings) {
...
std::vector<PrivacyFinding> DedupeFindings(const std::vector<PrivacyFinding>& findings) {
...
RawStreamScan ScanStreamRaw(const std::filesystem::path& stream_path) {
    RawStreamScan scan;
...
ResolvedText ResolveTextValue(const nlohmann::json& value, const BlobStore& blobs,
    ...
    if (ref->size > options.max_resolved_blob_bytes) {
            ...
            for (const auto& block : event["content"]) {
                ...
                if (GetString(block, "type") == "text") {
                    const ResolvedText resolved = ResolveTextValue(
                        block.contains("text") ? block["text"] : nlohmann::json(), blobs, options);
                    if (!resolved.ok) {
...
std::vector<std::filesystem::path> CollectSessionStreams(const std::filesystem::path& session_dir) {
