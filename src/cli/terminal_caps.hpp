...
//
// 为什么不做 DA1/XTGETTCAP 主动询问:那些应答走 stdin,会跟流式期间的
// 按键监听线程、逐键编辑器抢同一条输入流(POSIX 重画诚实关掉就是为这
// 个)。所以这里只做"被动探测":平台 console 探针 + 环境变量 + 已知
// 终端名录,拿不准一律 Unknown。每项都带"来源"说明,/doctor 与
// /config 可贴进 bug 单。
//
