...
const Entry kZhCN[] = {
    ...
    {"help.usage",
     ...
     "  lubancode hook init <名字>  生成 Lua hook 脚手架(hook.json + main.lua + fixtures,手册见\n"
     "                              docs/features/hooks/);hook validate/test <目录> 校验与试跑\n"
    ...
    {"slash_help.body",
     ...
     "可用命令:\n"
     "派工任务书六件套模板(单子路径/范围红线/环境实情/纪律/完工标准/回报格式)见 "
     "docs/reference/agents.md;agent 工具的 template 参数同款套壳。\n"},
    {"slash_help.keys",
    ...
    {"status.mode_switch_hint", "Shift+Tab 切换到{0}"},
    {"status.skills_count", "{0} skills"},
    {"status.skills_hint", "快捷键"},
    {"spinner.cancel_hint", "esc 取消"},
    {"mode.default", "默认"},
    {"mode.accept_edits", "接受编辑"},
    {"mode.yolo", "YOLO"},
    {"mode.auto", "自动模式"},
    {"mode.dont_ask", "不询问"},
    {"mode.notice.default", "默认：需要授权的操作会先询问你。"},
    {"mode.notice.accept_edits", "接受编辑：文件创建与改写自动允许；其他操作仍会询问。"},
    {"mode.notice.yolo", "YOLO：允许所有工具，无需确认。"},
    {"mode.notice.auto", "自动模式：文件编辑与安全命令自动允许；其余操作仍会询问。"},
    {"mode.notice.dont_ask", "不询问：已授权的操作照常执行；需要询问的操作直接拒绝。"},
    {"mode.start.deprecated_alias", "{0}=confirm 是旧名,已按 default 处理;新值请写 default。"},
    {"mode.start.invalid_value", "{0} 值无效: {1};已使用 default。"},
    {"mode.start.diagnostic", "审批档(起手) = {0}  [{1}]"},
    {"mode.start.diagnostic_session", "审批档(本会话) = {0}  [用户切档]"},
    {"mode.start.source_cli", "CLI --yes"},
    {"mode.start.source_env", "env LUBANCODE_CONFIRM_MODE={0}"},
    {"mode.start.source_project", "project permissions.default_confirm_mode={0}"},
    {"mode.start.source_builtin", "内置默认"},
    ...
    {"queue.mark.claimed", "[送达中] "},
    {"queue.mark.expired", "[已过期·未赶上本轮] "},
    {"queue.steer_expired_head", "{0} 条排队消息未赶上本轮(没有下一次模型请求),留在队里等你处置:取回改写再排,或删除。不会自动转成下一轮输入。"},
    {"queue.followup_suppressed", "上一轮被打断,排队的 followup 不自动续轮(要用就重新发送)。"},
    ...
    {"agent_status.reason_time_budget", "时间耗尽"},
    {"agent_status.reason_token_budget", "token 耗尽"},
    ...
    // 监督小截(监督器单 P1-1 §十):Dock 每行一小截——重连次数/静默龄,
    // 查看态的监督折叠行。只报计数与稳定码,不进思考与正文。
    {"agent_supervision.recovering", "重连 {0}/{1}"},
    {"agent_supervision.silent", "静默 {0}"},
    {"agent_supervision.degraded", "降级"},
    {"agent_supervision.age_never", "从未"},
    {"agent_supervision.no_reason", "无"},
    {"agent_supervision.wall_none", "不限"},
    {"agent_supervision.wall_over", "已到线"},
    {"agent_supervision.detail",
     "监督 · 最后传输 {0} · 最后完整提交 {1} · 实质进展 {2} · 请求尝试 {3} · 错误码 {4} · 墙钟剩余 {5}"},
    // agent_watch(监督器单 P1-0):模型可见的失败文案两语各一份。
    {"agent_watch.unavailable", "agent_watch 不可用:本会话没有子代理台账。"},
    {"agent_watch.invalid", "agent_watch 入参必须是 JSON 对象。"},
    {"agent_watch.unknown_key", "agent_watch 不认识入参键 \"{0}\":只认 task_ids/after_revision/wait_ms/include。"},
    {"agent_watch.task_ids_invalid", "task_ids 须是任务号数组(每项为不小于 1 的整数)。"},
    {"agent_watch.too_many_tasks", "task_ids 一次最多 {0} 只。"},
    {"agent_watch.after_revision_invalid", "after_revision 须是不小于 0 的整数。"},
    {"agent_watch.wait_ms_invalid", "wait_ms 须是 0~30000 的整数(毫秒)。"},
    {"agent_watch.include_invalid", "include 只认 summary/events/diagnostic。"},
    {"agent_watch.diagnostic_denied", "diagnostic 档只对 main 开放;子代理请用 summary 或 events。"},
    {"agent_watch.not_found", "没有任务 #{0}。"},
    {"agent_watch.not_child", "无权查看任务 #{0}:子代理只能看自己的直接子任务。"},
    ...
    {"help.scene_header", "当前场景按键(再按 {0} 或 Esc 收起;固定键是编辑器安全所需,不可改绑)"},
    {"help.scene_footer", "再按 {0} 或 Esc 收起;键位可用 /keymap set <动作> <和弦> 改绑,/keymap reset all 复位。"},
    ...
    {"slash.desc.package", "Package 只读面:list 列四层全账,show 看一只,doctor 诊一只(不挂组件)"},
    {"slash.desc.evolve", "自进化闭环:status 采集观察,list 按指纹聚类,propose 起草候选(簇够门槛出组合包),test 评测,approve/use/promote/rollback 批准与灰度,suggest 有限自动建议(缺省关)"},
    {"slash.desc.channels", "列渠道账号:全局配置里的渠道 × 本进程运行态(普通交互进程不启 sidecar,只读)"},
    {"slash.desc.channel", "/channel show|doctor|start|stop|restart <渠道> [账号]:单渠道账号管理面(多渠道接入阶段 2)"},
    ...
    // ---- /context-window 交互面板(ContextWindow交互面板单) ----
    ...
    {"cw_panel.unsaved", "未保存"},
    {"cw_panel.footer_nav", "↑↓ 选择设置项   ←→ 调值"},
    {"cw_panel.footer_save", "Enter 保存          Esc 取消(Ctrl+C 同取消)"},
    {"cw_panel.provider_default", "Provider 默认"},
    {"cw_panel.disabled", "关闭"},
    {"cw_panel.default_suffix", "(默认)"},
    {"cw_panel.unverified", "未验证"},
    {"cw_panel.over_limit", "超出已知上限"},
    {"cw_panel.not_supported", "此模型声明不吃推理参数,不可调"},
    {"cw_panel.always_on", "思考始终开启,关不掉"},
    {"cw_panel.unknown_caps", "能力未知,不猜档位"},
    {"cmd.context_window.usage", "用法: /context-window(裸敲开面板,同屏调当前模型的窗口与思考强度;本会话生效)"},
    {"cmd.context_window.non_tty",
     "非交互终端开不了 /context-window 面板。改窗口: /context <档位>(如 /context 256k);改思考: /think <档位>。均只本会话生效。"},
    {"cmd.context_window.cancelled", "已取消,窗口与思考强度未改动。"},
    {"cmd.context_window.effort_cleared", "推理强度已清为未发送参数(Provider default;本会话生效)。"},
    ...
    {"slash.desc.instructions", "看 AGENTS.md 指令链逐文件账（path <路径> / reload）"},
    {"slash.desc.telemetry", "本地遥测状态面(只读,不发请求);/telemetry [status]"},
    ...
    {"slash.desc.context_window", "同屏调当前模型的上下文窗口与思考强度(交互面板;本会话生效)"},
    ...
    // ---- slash 二级子命令补全说明(多级 Tab 补全) ----
    {"slash.sub.provider.list", "列出已配置的模型服务端"},
    {"slash.sub.provider.refresh", "重新探测当前服务端可用模型"},
    {"slash.sub.provider.add", "新增一个模型服务端(裸敲进分步向导)"},
    {"slash.sub.provider.switch", "切到另一个已配置的服务端"},
    {"slash.sub.provider.remove", "删除一个已配置的服务端"},
    {"slash.sub.provider.set", "改某个服务端的字段(如 native_web_search)"},
    {"slash.sub.provider.edit", "进向导改一个已配置的服务端"},
    {"slash.sub.instructions.path", "看某个文件/目录的 AGENTS.md 指令链"},
    {"slash.sub.instructions.reload", "重新加载当前 AGENTS.md 指令链"},
    {"slash.sub.goal.status", "看当前持久目标的进度"},
    {"slash.sub.goal.pause", "暂停当前持久目标"},
    {"slash.sub.goal.resume", "续跑已暂停的持久目标"},
    {"slash.sub.goal.clear", "清除当前持久目标"},
    {"slash.sub.goal.edit", "改当前持久目标的正文"},
    {"slash.sub.loop.list", "列出本会话建过的定时循环"},
    {"slash.sub.loop.status", "看某个定时循环任务的状态"},
    {"slash.sub.loop.pause", "暂停某个定时循环任务"},
    {"slash.sub.loop.resume", "续跑某个已暂停的定时循环任务"},
    {"slash.sub.loop.stop", "停止某个定时循环任务"},
    {"slash.sub.loop.run", "立即补跑一拍某个定时循环任务"},
    {"slash.sub.plan.status", "看当前 Plan 模式的状态"},
    {"slash.sub.plan.off", "退出 Plan 模式"},
    {"slash.sub.plan.review", "重开审阅上一次提交的计划"},
    {"slash.sub.record.status", "看当前录制的状态"},
    {"slash.sub.record.start", "开始录一遍生成技能"},
    {"slash.sub.record.note", "给当前录制加一条备注"},
    {"slash.sub.record.pause", "暂停当前录制"},
    {"slash.sub.record.resume", "续录已暂停的录制"},
    {"slash.sub.record.stop", "结束录制,出草稿"},
    {"slash.sub.record.cancel", "取消当前录制,不出草稿"},
    {"slash.sub.record.list", "列出本地录制件"},
    {"slash.sub.record.install", "把一份录制件确认安装为技能"},
    {"slash.sub.record.discard", "丢弃一份未安装的录制件"},
    ...
    {"slash.desc.insights",
     "/insights [--since 30d] [--sessions 200] [--all-workspaces]|status|clean --derived-only:跨会话/跨工作区洞察报告(本地确定性,零模型请求)"},

    // ---- /insights(Token 账本单 A5) ----
    {"cmd.insights.usage_line",
     "用法: /insights [--since 30d] [--sessions 200] [--all-workspaces] [--include-active] "
     "[--json] | /insights status | /insights clean --derived-only"},
    {"cmd.insights.unknown_arg", "认不得的参数。"},
    {"cmd.insights.ledger_off",
     "洞察报告账还没有可扫的 Journal 条目(本场还没跑过请求)。跑过几轮后再来。"},
    {"cmd.insights.later_model_review",
     "--model-review 属后续批次 A6:模型评议要显式确认并反向入账,本批本地报告照出,不调模型。"},
    {"cmd.insights.later_open",
     "--open 属后续批次 A7:报告文件的打开 containment 与 GUI 承载随后落。本批只打印路径,不自动打开。"},
    {"cmd.insights.no_home", "主目录解析不到,报告无处落。"},
    {"cmd.insights.write_failed", "报告写盘失败,已停(临时文件已清,Journal 不受影响): {0}"},
    {"cmd.insights.clean_needs_flag",
     "clean 只认 --derived-only(先列账再确认;Journal 永不在清理面)。"},
    {"cmd.insights.clean_confirm", "确认删除以上派生摘要? [y/N]: "},
    {"cmd.insights.clean_cancelled", "未删。派生摘要原样保留。"},
    {"cmd.insights.clean_done", "已删 {0} 个文件,共 {1}。Journal 与历史报告未动。"},
    ...
    {"cmd.usage.later_scope",
     "跨场汇总(day/week/workspace/all)属后续批次,本批只有会话级账。"},
    {"cmd.usage.flag_off",
     "Journal 侧逐请求账暂不可得,下面是本会话内存粗账(按角色累计,含当前回合,重启即清):"},
    {"cmd.usage.memory_caveat",
     "口径说明:内存粗账没有 purpose 分账、没有逐请求 coverage,也不含旁路请求全账;/usage 的逐请求实测账以 Journal 为准。"},
    ...
    {"config.output_source.subagent_default", "子任务受控默认(能力声明超窗收窄)"},
    ...
    {"status.cache_note_detail_unreported", "缓存明细未报"},
    {"status.cache_note_anomalous", "缓存账目异常"},
    {"status.cache_note_disabled", "服务端未启用缓存"},
    {"status.cache_note_upstream_miss", "缓存上游未命中(本地前缀稳定)"},
    ...
    {"cmd.skill.list_header", "本机技能: {0} 个生效"},
    {"cmd.skill.shadowed_total", "，另有 {0} 个被遮蔽"},
    {"cmd.skill.shadowed_by", "被 {0} 遮蔽"},
    {"cmd.skill.local", "本地自建"},
    {"cmd.skill.remote", "远端 {0}，安装于 {1}"},
    {"cmd.skill.scope_official", "官方"},
    {"cmd.skill.scope_agents", "agents 共享"},
    ...
    // /context 精细分类明细(ContextBreakdownDetail,全量对齐截图式占用
    // 面板):系统提示/工具定义两个粗桶的缩进子行 + 缓冲/空闲两行。
    {"cmd.context.bd.system_prompt_only", "系统提示正文"},
    {"cmd.context.bd.skills", "技能目录"},
    {"cmd.context.bd.system_tools", "系统工具"},
    {"cmd.context.bd.system_tools_deferred", "系统工具(延迟)"},
    {"cmd.context.bd.mcp_tools", "MCP 工具"},
    {"cmd.context.bd.mcp_tools_deferred", "MCP 工具(延迟)"},
    {"cmd.context.bd.plugin_tools", "插件工具"},
    {"cmd.context.bd.plugin_tools_deferred", "插件工具(延迟)"},
    {"cmd.context.bd.mcp_server_line", "· {0}:{1} 个工具,{2}"},
    {"cmd.context.bd.mcp_server_unknown", "(未知 server)"},
    {"cmd.context.bd.autocompact_buffer", "自动压缩缓冲"},
    {"cmd.context.bd.free_space", "空闲空间"},
    ...
    {"cmd.context.cache_history_capped", "已达环形上限:仅保留最近 {0} 次,全会话共 {1} 次模型请求"},
    {"cmd.context.cache_history_turn", "用户轮次 #{0}({1}):{2}"},
    {"cmd.context.cache_history_turn_nolabel", "未登记用户输入"},
    {"cmd.context.cache_history_turn_plain", "轮次 {0}"},
    {"cmd.context.cache_history_turn_unknown", "轮次不明(请求事件未带 turn_id)"},
    {"cmd.context.cache_history_row", "请求 {0}  输入 {1} / 命中 {2}({3}%)  ·  {4}"},
    {"cmd.context.cache_history_row_unreported", "请求 {0}  未回报(该次请求未带回 usage,另记缺测)  ·  {1}"},
    {"cmd.context.cache_history_row_no_ratio", "请求 {0}  比例未定(读取明细未报或账目异常,数字保留在案)  ·  {1}"},
    // 问题 9:每请求行的本地前缀视角段与 miss 分型短词。
    {"cmd.context.cache_diag_absent", "诊断未随行(该路径没带前缀诊断)"},
    {"cmd.context.cache_diag_epoch", "epoch {0}"},
    {"cmd.context.cache_diag_epoch_broken", "epoch {0}(断:{1})"},
    {"cmd.context.cache_diag_append", "前缀追加"},
    {"cmd.context.cache_diag_broken", "前缀断"},
    {"cmd.context.cache_diag_stable", "稳定前缀 {0}/{1} 条"},
    {"cmd.context.cache_diag_wire", "wire 前缀 {0}B"},
    {"cmd.context.cache_miss.hit", "命中"},
    {"cmd.context.cache_miss.first", "首请求"},
    {"cmd.context.cache_miss.epoch_break", "断 epoch"},
    {"cmd.context.cache_miss.upstream", "上游未命中(本地前缀稳定)"},
    {"cmd.context.cache_miss.unreported", "未回报"},
    {"cmd.context.cache_miss.detail_unreported", "缓存明细未报"},
    {"cmd.context.cache_miss.anomalous", "账目异常"},
    ...
    {"cmd.compact.dryrun.turnplan", "compact turn 计划:共 {0} turn · 按 L1 工作视图 token 平衡切成 {1} 份,预计 map {2} 次 + final reduce 1 次(不调模型,以下全是算出来的账)"},
    {"cmd.compact.dryrun.prior", "旧存档已剥出 ~{0} tokens:不算 turn,只进 final reduce 当基线"},
    {"cmd.compact.dryrun.partition", "  {0} = turn {1}-{2} · ~{3} tokens · 外置 ToolResult {4} 枚{5}"},
    {"cmd.compact.dryrun.hot_tag", " · 热区(保留原文形状)"},
    {"cmd.compact.dryrun.over_tag", " · 超 map 预算,map 时须沿 turn 边界再切"},
    {"cmd.compact.dryrun.offload", "长 ToolResult 外置前后:全量 ~{0} → 工作视图 ~{1} tokens(已外置 {2} 枚;分区按工作视图计量)"},
    {"cmd.compact.dryrun.over_partition", "警告:有冷区超 compact 模型输入预算(~{0}),map 时须沿 turn 边界再切,map 次数可多于计划"},
    {"cmd.compact.dryrun.over_turn", "警告:单枚 turn 超 compact 模型输入预算(~{0}),该次 compact 须明确拒绝——不截半条用户输入,不拆工具原子组"},
    {"cmd.compact.dryrun.orphan", "提示:历史里有不完整工具组(orphan tool_use 或悬空 result);分区边界只落 turn 之间,组不会被切开"},
    {"cmd.compact.dryrun.no_gain", "没有冷区也没有旧存档:压缩榨不出收益,应拒绝空跑(单 turn 场景保留原文即可)"},
    ...
    // ---- /doctor search(搜索兜底单:rg 三层健康检查) ----
    {"doctor.search.header", "search 后端(内置 ripgrep,三层发现):"},
    {"doctor.search.pinned", "钉死版本: {0}(命中件仍过 --version 精确门)"},
    {"doctor.search.tier.bundled", "随包(exe 旁 libexec)"},
    {"doctor.search.tier.stage", "用户级 rg-stage"},
    {"doctor.search.tier.path", "系统 PATH"},
    {"doctor.search.state.missing", "缺"},
    {"doctor.search.state.not_executable", "在,不可执行"},
    {"doctor.search.state.hit", "命中,实测 {0}, {1}"},
    {"doctor.search.state.usable_not_hit", "在,可用(命中在更前层,未用)"},
    {"doctor.search.state.path_none", "无可用 rg(共 {0} 项)"},
    {"doctor.search.current_hit", "当前用: 第 {0} 层({1})"},
    {"doctor.search.current_missing", "当前用: 无——三层全缺,search 报稳定错 search_backend_missing"},
    {"doctor.search.fix",
     "修复: bash scripts/fetch_ripgrep.sh --platform <平台>(--target 缺省自动认数据根:应用根=LUBANCODE_DATA_HOME 或 <LUBANCODE_HOME>/data,个人布局=<home>/.lubancode/rg-stage;补齐后自动兜底,无需随包)"},
    {"doctor.search.note",
     "注: 兜底顺序 随包 → rg-stage → PATH;兜底直接用原路径不拷贝,命中件照过版本门,过不了报对应的错。"},
    ...
    {"doctor.cache.probe_rounds_capped", "轮数超过上限,按 {0} 轮跑。"},
    {"doctor.cache.probe_disclosure_header", "公网探针确认门:接下来的动作会真实发送请求。"},
    {"doctor.cache.probe_disclosure_body",
     "将向 {0} 发 {1} 轮固定前缀探针(每轮 1 枚小请求):同一段诊断短句反复发送,不含会话历史;预计输入上限 {2} token/轮、输出上限 {3} token/轮(保守写死,不加码)。"},
    {"doctor.cache.probe_confirm_prompt", "确认发送?(y/N)"},
    {"doctor.cache.probe_declined", "未确认,不发探针。想免确认:provider 配 metrics_url 声明自有端,或改用本机回环端点。"},
    {"doctor.cache.probe_round", "第 {0} 轮(同 system、同历史前缀,只换最后一句):"},
    {"doctor.cache.probe_usage", "  usage:完整输入 {0} · cached_tokens 命中 {1}"},
    {"doctor.cache.probe_prefix", "前缀字节:相邻两轮请求体公共前缀最小 {0} 字节(设计前缀 {1} 字节)"},
    {"doctor.cache.probe_prefix_stable", " —— 稳定"},
    {"doctor.cache.probe_prefix_broken", " —— 不稳(公共前缀比设计短,序列化在改写前缀)"},
    {"doctor.cache.probe_verdict", "分型:{0}"},
    {"doctor.cache.probe_evidence_bound",
     "证据边界:无 metrics 权限,判断只凭各轮 usage 回报与本地 wire 公共前缀字节;上游节点亲和、缓存寿命与逐出策略在这份账里看不见,分型说的是\"这几次请求的形状\",不是服务端配置的定论。"},
    {"doctor.cache.verdict.stable_hit", "稳定命中(各轮都命中,且吃满设计前缀)"},
    {"doctor.cache.verdict.fixed_quantum", "固定阈值命中(各轮都命中,但命中量恒定、低于设计前缀——上游只肯缓存一只固定的块)"},
    {"doctor.cache.verdict.intermittent", "间歇 miss(有的轮命中、有的轮零,或命中量在抖)"},
    {"doctor.cache.verdict.no_hit", "完全未见命中(报了 usage 的轮全零)"},
    {"doctor.cache.verdict.not_reported", "无法判定(没有一轮回报 usage)"},
    ...
    {"cmd.soul.status.locked", "锁定状态:已锁定(自首次请求起固定,本会话不再换魂;改的只留默认值)。"},
    {"cmd.soul.status.unlocked", "锁定状态:未锁定(首次请求发出前还可改本会话魂)。"},
    {"cmd.soul.default_header", "下个新会话默认值: {0}"},
    {"cmd.soul.pending_note", "默认值与当前快照不同: {0} 将在下个新会话生效,本会话不切换。"},
    {"cmd.soul.pending_same", "默认值与当前快照相同。"},
    {"cmd.soul.locked.save_hint", "已保存,下个新会话生效。本会话仍用「{0}」。中途更换 Soul 会改写系统提示词,影响后续缓存复用,因此本会话不切换。"},
    {"cmd.soul.draft.save_hint", "已设置。本会话从首次请求起固定使用这份 Soul;此后修改留到下个新会话。"},
    {"cmd.soul.unchanged", "内容未变。"},
    ...
    {"cmd.sessions.query_failed", "会话列表读不成,以下为能读到的部分: {0}"},
    {"cmd.sessions.kind_unknown", "      (种类未知:老档没写 runKind,能否续聊以 /resume 七步验账为准)"},
    ...
    {"cmd.resume.query_failed", "存档列表读不成: {0}。先看数据根是否可读(/doctor),修好再试恢复。"},
    ...
    {"cmd.resume.history.compact_tokens", "── 此处发生过一次上下文压缩:前 ~{0} → 后 ~{1} tokens ──"},
    ...
    {"picker.empty.error", "会话列表读不成: {0}(Tab 切 All 再试,或跑 /doctor 看数据根)。"},
    ...
    {"picker.transcript.scroll_footer",
     "esc/ctrl+t 收起 · enter resume · ↑↓ 滚动 · pgup/pgdn 翻页 · 上方{0} · 下方{1}"},
    {"picker.transcript.more_older", "还有更早"},
    {"picker.transcript.no_more_older", "已到最早"},
    {"picker.transcript.more_newer", "还有更新"},
    {"picker.transcript.no_more_newer", "已是最新"},
    {"picker.footer",
...
const Entry kEn[] = {
    ...
    {"help.usage",
     ...
     "  lubancode hook init <name>\n"
     "                              scaffold a Lua hook package (hook.json + main.lua + fixtures;\n"
     "                              manuals under docs/features/hooks/); hook validate/test <dir>\n"
     "                              checks and runs fixtures\n"
    ...
    {"slash_help.body",
     ...
     "Available commands:\n"
     "For the six-part dispatch task-brief template (ticket path / scope red lines / environment\n"
     "facts / discipline / done criteria / report format) see docs/reference/agents.md; the agent\n"
     "tool's template parameter wraps the same shell.\n"},
    {"slash_help.keys",
    ...
    {"status.mode_switch_hint", "Shift+Tab to {0}"},
    {"status.skills_count", "{0} skills"},
    {"status.skills_hint", "keys"},
    {"spinner.cancel_hint", "esc to cancel"},
    {"mode.default", "Default"},
    {"mode.accept_edits", "Accept Edits"},
    {"mode.yolo", "YOLO"},
    {"mode.auto", "Auto Mode"},
    {"mode.dont_ask", "Don't Ask"},
    {"mode.notice.default", "Default: asks before operations that need approval."},
    {"mode.notice.accept_edits", "Accept Edits: allows file creation and edits; asks for other operations."},
    {"mode.notice.yolo", "YOLO: allows all tools without approval."},
    {"mode.notice.auto", "Auto Mode: allows file edits and safe commands; asks for the rest."},
    {"mode.notice.dont_ask", "Don't Ask: runs pre-approved operations and denies anything that would need a prompt."},
    {"mode.start.deprecated_alias", "{0}=confirm is a legacy name, read as default; write default instead."},
    {"mode.start.invalid_value", "Invalid {0} value: {1}; using default."},
    {"mode.start.diagnostic", "approval mode (startup) = {0}  [{1}]"},
    {"mode.start.diagnostic_session", "approval mode (session) = {0}  [switched by user]"},
    {"mode.start.source_cli", "CLI --yes"},
    {"mode.start.source_env", "env LUBANCODE_CONFIRM_MODE={0}"},
    {"mode.start.source_project", "project permissions.default_confirm_mode={0}"},
    {"mode.start.source_builtin", "built-in default"},
    ...
    {"queue.mark.claimed", "[delivering] "},
    {"queue.mark.expired", "[expired · missed this turn] "},
    {"queue.steer_expired_head",
     "{0} queued message(s) missed this turn (no further model request); kept in the queue for you to edit and re-queue, or delete. They will NOT auto-start a next turn."},
    {"queue.followup_suppressed", "Previous turn was interrupted; queued followups do not auto-start a new turn (re-send if you want them)."},
    ...
    {"agent_status.reason_time_budget", "time budget exhausted"},
    {"agent_status.reason_token_budget", "token budget exhausted"},
    ...
    // Supervision snippet (supervisor ticket P1-1 §10): one short segment per
    // dock line — retry count / silence age; the folded supervision row in the
    // detail view. Counts and stable codes only, never content.
    {"agent_supervision.recovering", "retry {0}/{1}"},
    {"agent_supervision.silent", "silent {0}"},
    {"agent_supervision.degraded", "degraded"},
    {"agent_supervision.age_never", "never"},
    {"agent_supervision.no_reason", "none"},
    {"agent_supervision.wall_none", "none"},
    {"agent_supervision.wall_over", "over"},
    {"agent_supervision.detail",
     "supervision · transport {0} ago · commit {1} ago · progress {2} ago · attempt {3} · err {4} · wall {5} left"},
    // agent_watch (supervisor ticket P1-0): model-visible failure texts.
    {"agent_watch.unavailable", "agent_watch unavailable: this session has no subagent ledger."},
    {"agent_watch.invalid", "agent_watch input must be a JSON object."},
    {"agent_watch.unknown_key", "agent_watch does not know key \"{0}\": only task_ids/after_revision/wait_ms/include."},
    {"agent_watch.task_ids_invalid", "task_ids must be an array of task numbers (integers >= 1)."},
    {"agent_watch.too_many_tasks", "task_ids accepts at most {0} tasks per call."},
    {"agent_watch.after_revision_invalid", "after_revision must be an integer >= 0."},
    {"agent_watch.wait_ms_invalid", "wait_ms must be an integer between 0 and 30000 (milliseconds)."},
    {"agent_watch.include_invalid", "include only accepts summary/events/diagnostic."},
    {"agent_watch.diagnostic_denied", "diagnostic is main-only; subagents use summary or events."},
    {"agent_watch.not_found", "no task #{0}."},
    {"agent_watch.not_child", "no access to task #{0}: a subagent can only watch its own direct children."},
    ...
    {"help.scene_header", "Keys for this scene (press {0} or Esc to dismiss; fixed keys protect the editor)"},
    {"help.scene_footer", "Press {0} or Esc to dismiss; rebind with /keymap set <action> <chord>, reset with /keymap reset all."},
    ...
    {"slash.desc.insights",
     "/insights [--since 30d] [--sessions 200] [--all-workspaces]|status|clean --derived-only: cross-session/cross-workspace insight report (local deterministic, zero model requests)"},

    // ---- /insights (token ledger ticket A5) ----
    {"cmd.insights.usage_line",
     "Usage: /insights [--since 30d] [--sessions 200] [--all-workspaces] [--include-active] "
     "[--json] | /insights status | /insights clean --derived-only"},
    {"cmd.insights.unknown_arg", "Unknown argument."},
    {"cmd.insights.ledger_off",
     "The journal has no scannable entries yet (no requests run in this workspace). Run a few turns first."},
    {"cmd.insights.later_model_review",
     "--model-review lands in batch A6: model review requires explicit consent and is journaled back; this batch ships the local report and calls no model."},
    {"cmd.insights.later_open",
     "--open lands in batch A7 (open containment + GUI host). This batch prints report paths only, nothing auto-opens."},
    {"cmd.insights.no_home", "Cannot resolve the home directory; nowhere to write reports."},
    {"cmd.insights.write_failed", "Report write failed, stopped (temp files cleaned, journal untouched): {0}"},
    {"cmd.insights.clean_needs_flag",
     "clean only accepts --derived-only (list first, then confirm; the journal is never in scope)."},
    {"cmd.insights.clean_confirm", "Delete the derived summaries listed above? [y/N]: "},
    {"cmd.insights.clean_cancelled", "Nothing deleted. Derived summaries left as-is."},
    {"cmd.insights.clean_done", "Deleted {0} files, {1}. Journal and report history untouched."},
    ...
    {"slash.desc.package", "read-only Package views: list all layers, show one, doctor one (no mounting)"},
    {"slash.desc.evolve", "self-evolution loop: status collects observations, list clusters by fingerprint, propose drafts candidates (combo package once the cluster clears the bar), test evaluates, approve/use/promote/rollback gate & roll, suggest nudges (off by default)"},
    {"slash.desc.channels", "list channel accounts: global config entries x this process runtime state (interactive mode never spawns sidecars; read-only)"},
    {"slash.desc.channel", "/channel show|doctor|start|stop|restart <channel> [account]: per-account channel management (channels stage 2)"},
    ...
    {"agent_panel.budget_head", "budget: {0}"},
    ...
    // ---- /context-window panel (context-window interactive panel) ----
    // The panel only changes this session's window budget and thinking effort;
    ...
    {"cw_panel.unsaved", "Unsaved"},
    {"cw_panel.footer_nav", "Up/Down select setting   Left/Right change value"},
    {"cw_panel.footer_save", "Enter Save          Esc Cancel (Ctrl+C also cancels)"},
    {"cw_panel.provider_default", "Provider default"},
    {"cw_panel.disabled", "Disabled"},
    {"cw_panel.default_suffix", "(Default)"},
    {"cw_panel.unverified", "unverified"},
    {"cw_panel.over_limit", "exceeds known limit"},
    {"cw_panel.not_supported", "Not supported (model declines reasoning parameters)"},
    {"cw_panel.always_on", "Always enabled, cannot be turned off"},
    {"cw_panel.unknown_caps", "Capabilities unknown; no guessed levels"},
    {"cmd.context_window.usage",
     "Usage: /context-window (bare opens the panel for this model's window and effort; session only)"},
    {"cmd.context_window.non_tty",
     "The /context-window panel needs an interactive terminal. Change the window with "
     "/context <size> (e.g. /context 256k) and the effort with /think <level>; both session only."},
    {"cmd.context_window.cancelled", "Cancelled; window and thinking effort unchanged."},
    {"cmd.context_window.effort_cleared", "Reasoning effort cleared to no parameter (provider default; session only)."},
    {"cmd.context_window.reject.model_changed",
    ...
    {"cmd.context_window.reject.history_conflict",
     "Conflict: cross-turn preserved thinking (history all) requires thinking to be on; this model "
     "cannot preserve with thinking off. Run /think history default before disabling, or pick an on level."},
    ...
    {"slash.desc.instructions", "show the AGENTS.md instruction chain per file (path <path> / reload)"},
    {"slash.desc.telemetry", "local telemetry status (read-only, no requests); /telemetry [status]"},
    ...
    {"slash.desc.context_window", "Set context window for the current model"},
    ...
    {"cmd.sessions.archived_hint", "These are archived, bytes untouched (read-only view)."},
    ...
    // ---- slash subcommand completion descriptions (multi-level Tab completion) ----
    {"slash.sub.provider.list", "list configured model providers"},
    {"slash.sub.provider.refresh", "re-probe the current provider's available models"},
    {"slash.sub.provider.add", "add a new model provider (bare word opens the step-by-step wizard)"},
    {"slash.sub.provider.switch", "switch to another configured provider"},
    {"slash.sub.provider.remove", "remove a configured provider"},
    {"slash.sub.provider.set", "change a provider field (e.g. native_web_search)"},
    {"slash.sub.provider.edit", "open the wizard to edit a configured provider"},
    {"slash.sub.instructions.path", "show the AGENTS.md instruction chain for a file or directory"},
    {"slash.sub.instructions.reload", "reload the current AGENTS.md instruction chain"},
    {"slash.sub.goal.status", "show the current durable goal's progress"},
    {"slash.sub.goal.pause", "pause the current durable goal"},
    {"slash.sub.goal.resume", "resume a paused durable goal"},
    {"slash.sub.goal.clear", "clear the current durable goal"},
    {"slash.sub.goal.edit", "edit the current durable goal's text"},
    {"slash.sub.loop.list", "list recurring loops created in this session"},
    {"slash.sub.loop.status", "show a recurring loop task's status"},
    {"slash.sub.loop.pause", "pause a recurring loop task"},
    {"slash.sub.loop.resume", "resume a paused recurring loop task"},
    {"slash.sub.loop.stop", "stop a recurring loop task"},
    {"slash.sub.loop.run", "dispatch a recurring loop task right now"},
    {"slash.sub.plan.status", "show the current Plan mode status"},
    {"slash.sub.plan.off", "exit Plan mode"},
    {"slash.sub.plan.review", "reopen review of the last submitted plan"},
    {"slash.sub.record.status", "show the current recording status"},
    {"slash.sub.record.start", "start recording a workflow into a skill"},
    {"slash.sub.record.note", "add a note to the current recording"},
    {"slash.sub.record.pause", "pause the current recording"},
    {"slash.sub.record.resume", "resume a paused recording"},
    {"slash.sub.record.stop", "stop recording and draft the skill"},
    {"slash.sub.record.cancel", "cancel the current recording without drafting"},
    {"slash.sub.record.list", "list local recordings"},
    {"slash.sub.record.install", "install a recording as a skill after review"},
    {"slash.sub.record.discard", "discard an uninstalled recording"},
    ...
    {"cmd.usage.later_scope",
     "Cross-session rollups (day/week/workspace/all) ship in a later batch; this batch is per-session only."},
    {"cmd.usage.flag_off",
     "Per-request journal tally is unavailable; below is the in-memory tally for this session (per-role, includes the current turn, lost on restart):"},
    {"cmd.usage.memory_caveat",
     "Caveat: the in-memory tally has no purpose split, no per-request coverage, and is not the full bypass ledger. The per-request journal report is authoritative."},
    ...
    {"cmd.soul.current", "This session's soul: {0}"},
    ...
    {"cmd.soul.status.locked", "Lock state: locked (fixed since the first request; this session keeps it; later edits only change the default)."},
    {"cmd.soul.status.unlocked", "Lock state: unlocked (this session's soul can still change before the first request goes out)."},
    {"cmd.soul.default_header", "Default for the next new session: {0}"},
    {"cmd.soul.pending_note", "The default differs from the current snapshot: {0} takes effect in the next new session, not this one."},
    {"cmd.soul.pending_same", "The default matches the current snapshot."},
    {"cmd.soul.locked.save_hint", "Saved; it takes effect in the next new session. This session keeps \"{0}\". Swapping the Soul mid-session rewrites the system prompt and hurts later cache reuse, so this session does not switch."},
    {"cmd.soul.draft.save_hint", "Set. This session uses this Soul from its first request; later edits wait for the next new session."},
    {"cmd.soul.unchanged", "Content unchanged."},
    ...
    {"config.output_source.subagent_default", "subagent capped default (capability declaration narrowed)"},
    ...
    // ---- /doctor search (search-fallback doc: three-tier rg health check) ----
    {"doctor.search.header", "search backend (bundled ripgrep, three-tier discovery):"},
    {"doctor.search.pinned", "pinned version: {0} (the winning binary still passes the exact --version gate)"},
    {"doctor.search.tier.bundled", "bundled (libexec beside the exe)"},
    {"doctor.search.tier.stage", "user rg-stage"},
    {"doctor.search.tier.path", "system PATH"},
    {"doctor.search.state.missing", "missing"},
    {"doctor.search.state.not_executable", "present, not executable"},
    {"doctor.search.state.hit", "hit — found {0}, {1}"},
    {"doctor.search.state.usable_not_hit", "present and usable (an earlier tier won, unused)"},
    {"doctor.search.state.path_none", "no usable rg ({0} entries scanned)"},
    {"doctor.search.current_hit", "in use: tier {0} ({1})"},
    {"doctor.search.current_missing", "in use: none — all three tiers missing, search fails with the stable error search_backend_missing"},
    {"doctor.search.fix",
     "fix: bash scripts/fetch_ripgrep.sh --platform <platform> (default --target resolves the data root: LUBANCODE_DATA_HOME or <LUBANCODE_HOME>/data under an app root, <home>/.lubancode/rg-stage otherwise; search then self-heals to it, no bundle needed)"},
    {"doctor.search.note",
     "note: fallback order bundled → rg-stage → PATH; the fallback path is used in place, never copied, and still passes the version gate — failing it reports the specific error."},
    ...
    {"cmd.resume.history.compact_tokens",
     "── Context was compacted here: before ~{0} -> after ~{1} tokens ──"},
    ...
    {"cmd.skill.list_header", "Local skills: {0} active"},
    {"cmd.skill.shadowed_total", ", {0} shadowed"},
    {"cmd.skill.shadowed_by", "shadowed by {0}"},
    {"cmd.skill.local", "local"},
    {"cmd.skill.remote", "remote {0}, installed {1}"},
    {"cmd.skill.scope_official", "official"},
    {"cmd.skill.scope_agents", "agents shared"},
    ...
    // ---- turn partition plan (compact quad-partition, stage 1): pure calculation, no model calls ----
    {"cmd.compact.dryrun.turnplan", "compact turn plan: {0} turns total · balanced into {1} partitions by L1 working-view tokens, ~{2} map calls + 1 final reduce (no model called; everything below is computed)"},
    {"cmd.compact.dryrun.prior", "Prior archive peeled off (~{0} tokens): not a turn; baseline input to the final reduce only"},
    {"cmd.compact.dryrun.partition", "  {0} = turns {1}-{2} · ~{3} tokens · {4} offloaded ToolResults{5}"},
    {"cmd.compact.dryrun.hot_tag", " · hot zone (raw message shapes kept)"},
    {"cmd.compact.dryrun.over_tag", " · over map budget; must be re-split along turn boundaries when mapping"},
    {"cmd.compact.dryrun.offload", "ToolResults before/after offload: raw ~{0} -> working view ~{1} tokens ({2} offloaded; partitions are measured on the working view)"},
    {"cmd.compact.dryrun.over_partition", "warning: a cold partition exceeds the compact model input budget (~{0}); re-split along turn boundaries when mapping, map calls may exceed the plan"},
    {"cmd.compact.dryrun.over_turn", "warning: a single turn exceeds the compact model input budget (~{0}); that compaction must be rejected outright — never truncate a user message or split a tool atomic group"},
    {"cmd.compact.dryrun.orphan", "note: incomplete tool exchanges present (orphan tool_use or dangling result); partition boundaries fall between turns only, groups are never split"},
    {"cmd.compact.dryrun.no_gain", "No cold region and no prior archive: compaction yields nothing; refuse the run (a single turn is best kept raw)"},
    ...
    {"picker.empty.error", "Session list unreadable: {0} (Tab to Filter, switch to All, or run /doctor)."},
    ...
    {"picker.transcript.scroll_footer",
     "esc/ctrl+t close · enter resume · up/down scroll · pgup/pgdn page · older: {0} · newer: {1}"},
    {"picker.transcript.more_older", "more"},
    {"picker.transcript.no_more_older", "at the oldest"},
    {"picker.transcript.more_newer", "more"},
    {"picker.transcript.no_more_newer", "at the newest"},
    {"picker.footer",
