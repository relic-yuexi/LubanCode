...
//
// Unicode 15.1(UCD 15.1.0)。两张内嵌表是机器从 UCD 官方数据生成的全量
// 区间表(scripts/gen_unicode_tables.py,来源 URL 与版本写死在脚本头,
// 表头注明生成日期;人工摘录表已于宽表机器生成单淘汰):
//   - 表一 Extend:UAX#29 Grapheme_Cluster_Break=Extend 全量(auxiliary/
//     GraphemeBreakProperty.txt;含变体选择符、ZWNJ、emoji 肤色修饰)。
//   - 表二 宽字:East_Asian_Width 的 W/F ∪ Emoji_Presentation。
...
// UCD 15.1.0 auxiliary/GraphemeBreakProperty.txt 的 Extend(含变体选择符、
// ZWNJ 与 emoji 肤色修饰),生成日期 2026-09-09;区间升序、互不重叠、相邻已
...
// 机器生成,勿手改:scripts/gen_unicode_tables.py --write 重生成。来源
// UCD 15.1.0 EastAsianWidth.txt(W/F,含头注释规定的未赋值码位默认 W 段)
// 与 emoji-data.txt(Emoji_Presentation),生成日期 2026-09-09;区间升序、互不
