...
public:
    struct Options {
        ...
        // 批次执行策略(只读并行单 P3 宿主接线):gateway/助理的自动任务
        // 会话从 config 的 agent.tool_execution/parallel_read_concurrency 折
        // 好递进(装配层 gateway_launch/assistant_tasks);缺省 Exclusive 与
        // AgentRuntimeProfile 同档——不声明不并行,行为与从前一字不差。
