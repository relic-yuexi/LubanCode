...
public:
    ...
    // 空 = 运行时不可用(没配子代理通道的入口),execute 明确报
    // unavailable,不假报已通知。caller_task_id:0 = main(不限定,可投
    // 任意存活任务);非 0 = 只挂给某只子代理的窄实例,只能投它自己的
    // 直接孩子。
    explicit AgentMessageTool(std::shared_ptr<AgentTaskCoordinator> coordinator, int caller_task_id = 0)
