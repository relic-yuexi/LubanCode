...
//
// 复用件(跨渠道中性,R0 后仍住 qq 命名空间的按原位借用,不重复造):
//   - QqSpoolStore:delivery_id+事件 JSON 的先落盘再上报账(路径自拼
//     <root>/feishu/<acct>/…,与 qq 互不相见);
//   - qq_adapter.hpp 的 ConnectionSnapshot:连接状态快照(字段全中性,
...
#include "channel/qq/qq_adapter.hpp"  // ConnectionSnapshot/ConnectionFailure(中性快照)
#include "channel/qq/qq_spool.hpp"    // 入站 spool(渠道中性账)
...
public:
    ...
    // 平台连接状态快照(字段中性,复用 qq 命名空间的结构;stage 用飞书的
    // 稳定名:bootstrapping/connecting/connected/stopped)。
    qq::ConnectionSnapshot ConnectionState() const;
