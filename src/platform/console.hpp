...
// 读缓冲区绝对行 row 的可见文本(行尾空格剥掉),改宽残帧清扫用:改宽后
// conhost 重排旧行,按模型算出来的旧帧位置可能对不上真实落点,须现读
// 现比对才能把"冻结的活动行"这类残影扫净。读不到(非真控制台/越界/
// 查询失败)返回 std::nullopt;POSIX 无缓冲区可读,恒 nullopt(清扫自然
// 退化为模型路,行为不劣化)。
