// turn_runner.hpp 的实现:PrintDivider/RunTurn 与控制面装配(BuildTurnWiring)
// 的函数体全在这只 translation unit 里,控制台读取、状态画板、分界线、图像
// 输入等终端依赖不往公开头漏。
//
// 骨架拆解反弹·问题 1:PromptAskUser(ask_user 问询)整段搬去
// cli/ask_user_prompt;确认的终端半边(参数详情/diff 预览/三档菜单/
// [y/a/N]/settings.local.json 持久化)搬去 cli/tool_confirm_ui。这里只剩
// 判断与拼单,不碰真终端 IO、不写配置文件。
...
// 确认问话的判断半边(骨架拆解反弹·问题 1:判断与渲染分家)。
// P3(显示系统剥离单):档位/黑名单/钩子表态的裁定已抽去
// runtime::EvaluatePermission 与 runtime::EmitPermissionRequest(纯逻辑,
// 可脱离终端单测)。本轮整改再把"把决定落到画面上"的活——diff 预览、
// 三档菜单、[y/a/N]、settings.local.json 追问与持久化——整段搬去
// cli/tool_confirm_ui:这里只拼一份 ToolConfirmRequest 递过去,不直接碰
// ReadChoiceMenu/ReadLine/ShowDiffPreview,也不写配置文件。裁定次序的
// 完整注释见 runtime/turn_runtime.hpp。
bool ConfirmToolUse(const std::string& tool_use_id, bool auto_confirm,
    ...
    // 拼单,交终端半边(cli/tool_confirm_ui):铺详情/diff、三档菜单或
    // [y/a/N] 读行、答完收画面、"允许并记住"的追问与持久化全在那边。
    // "sink 收到用户选择后调 OnToolAllowedPersist"的接线也在那只文件里,
    // 本文件不再 include config/config.hpp 写 settings.local.json。
    lubancode::cli::ToolConfirmRequest request;
    request.tool_use_id = tool_use_id;
    request.name = name;
    request.input = input;
    request.auto_confirm = auto_confirm;
    request.always_allowed = &always_allowed_tools;
    request.theme = &theme;
    request.display = &display;
    request.approval_observer = approval_observer;
    return lubancode::cli::AskToolConfirm(request);
}
...
lubancode::agent::TurnWiring BuildTurnWiring(TurnContext& ctx, ToolDisplay& display,
    ...
    if (lubancode::runtime::HasPreRequestMiddleware(hook_dispatcher)) {
        wiring.on_pre_request_hooks = [hook_dispatcher](const std::string& step_id, const std::string& turn_id,
                                                        ...
                                                        const lubancode::runtime::PreRequestBudget& budget) {
            lubancode::runtime::MiddlewareHookContext context;
            context.turn_id = turn_id;
            context.step_id = step_id;
            context.purpose = "interactive";
            const lubancode::runtime::PreRequestStages stages =
                lubancode::runtime::RunPreRequestMiddleware(hook_dispatcher, frozen_request_snapshot, budget,
                                                            context);
