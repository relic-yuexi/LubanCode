// interactive_session.hpp 的实现:InteractiveLoop 整段原样搬自原头文件,
// 行为一字未改;后续 commit 在这只 translation unit 里把大 lambda 收成
// 方法、局部状态收成成员。
...
void InteractiveLoop(lubancode::config::ConfigResult config_result, bool auto_confirm,
                      ...
                      const lubancode::config::SettingsLocal& settings_local, bool continue_last,
                      const std::string& law_source, const std::string& executable) {
