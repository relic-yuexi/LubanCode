// HC-06(把Slash命令材料收窄到各自领域)第一小批:命令绑定单元——
// 集中构造"捕获材料的执行器"。路由表(command_registry)只接命令参数,
// 各域 handler 只吃自己的窄材料;尚未收窄的域仍经 SlashDispatchContext
// 过渡(后续批次逐域迁走,最后删掉整束公共材料)。
//
// 分层规矩:
...
struct SessionCommandMaterials {
    // 尚未收窄域的过渡材料袋(HC-06 批2/批3 逐域搬走后删除;绑定期
    // 不可为空,对账钉子 SlashCommandTable() 喂空对象)。
    SlashDispatchContext* dispatch = nullptr;
    // 已收窄域的窄材料(HC-06 第一小批:Trace/Hook/Telemetry)。
