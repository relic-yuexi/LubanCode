...
//      本地假 Anthropic 后端),参考前端内核连上、握手 1.3;
...
async function main() {
  ...
  try {
    ...
    ok('握手:protocolVersion 1.3(承载同协议)', Boolean(init) && init.protocolVersion === '1.3',
    ...
    // ---- 4. token 门(口子与 WS 同规矩;另一台短命服务) ----
    {
      ...
      if (await waitReachable(gatePort)) {
        ...
        ok('token 门:内核首帧 auth 过门,业务照常', Boolean(gatedInit) && gatedInit.protocolVersion === '1.3');
