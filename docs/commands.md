...
在 Git 根生成 `AGENTS.md`；非 Git 目录写在 cwd。已有 `AGENTS.md` 或 `AGENTS.override.md` 时不覆盖，只重载。详见[项目指令](project-instructions.md)。
...
详见[会话与上下文](sessions-and-context.md)。
...
`on/off` 改本场总状态；`use` 控召回；`learn` 控学习档位(只能降到全局授权以内)。`review` 看待审候选,`accept`/`edit`/`reject` 处置。`remember` 排后台任务(feedback 只收用户明说的纠正)，`forget` 归档而非直接抹账，`rebuild` 从主题 Markdown 重建 catalog 与 index，`stale` 查陈旧，`verify`/`refresh` 续命，`migrate` 把旧格式主题批迁 front matter(先列账、备份、确认后动盘)，`why` 对上一轮召回对账。详见[项目记忆](memory-system-design.md)。
