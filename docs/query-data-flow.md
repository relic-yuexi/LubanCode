...
请求拼装前还有一道 mid-turn 评估：系统提示 + 工具定义 + 全份历史 + 输出预留（统一 token 口径：ASCII 4 字符约 1 token，非 ASCII 每字约 1.5 token）估过有效窗口的 80%，就在这个"工具结果已攒完、请求尚未发出"的安全点先做一次语义压缩，不再等下一条用户消息。轮级硬裁真丢了东西（丢轮或截结果）时，会向终端发一条显式告警——有损降级不许静默发生。
