...
[文档首页](README.md) · [开发指南](development-guide.md) · [测试指南](testing-guide.md) · [命名规范](naming-conventions.md)
...
| 类型 | 回答什么 | 例子 |
| --- | --- | --- |
| 入口 | 我该从哪儿开始 | `README.md`、`feature-reference.md` |
| 指南 | 怎样完成一件事 | `project-instructions.md`、`extensions.md` |
| 参考 | 字段、命令、参数究竟是什么 | `commands.md`、`configuration.md`、`tools.md` |
| 解释 | 系统为何这样长、数据怎样流 | `architecture.md`、`query-data-flow.md` |
| 规范 | 改动必须守哪些契约 | 本页、`naming-conventions.md`、`security-model.md` |
| 附录 | 某种场景下的素材或快照 | `job-portfolio.md` |
...
- 仓库内用相对链接：`[配置手册](configuration.md)`。
...
执行层级、计数单位先查[命名与计数规范](naming-conventions.md)。几条硬规矩：
...
| 改动 | 至少检查 |
| --- | --- |
| 启动参数 | `commands.md`、根 README、CLI help、CHANGELOG |
| Slash 命令或键位 | `commands.md`、`terminal-ui.md`、i18n/help、CHANGELOG |
| 配置字段或默认值 | `configuration.md`、示例配置、`--config` 输出、CHANGELOG |
| Provider/模型 schema | `provider-catalog.md`、`configuration.md`、catalog schema/快照 |
| 工具 schema | `tools.md`、`feature-reference.md`、prompt 工具方针、测试 |
| Hook 事件 | `hooks.md`、`configuration.md`、扩展指南、事件测试 |
| 会话/compact/memory | 对应专题页、`query-data-flow.md`、迁移/回放测试 |
| 终端状态机 | `terminal-ui.md`、`commands.md` 键位表、真终端测试说明 |
| 安全边界 | `security-model.md`、专题页风险段、确认与失败测试 |
| 构建/CI/发行 | `development-guide.md`、`testing-guide.md`、根 README |
...
纯文档改动不触发当前代码 CI（workflow 对 `docs/**` 做了 `paths-ignore`）。这不是免检牌。链接、示例与事实校验须在本地完成；文档若伴随代码改动，则照代码风险跑测试。
