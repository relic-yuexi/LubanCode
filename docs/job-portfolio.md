...
- [ ] **发出 `v0.24.0`。** README 与源码已写 0.24.0，公开 Release 仍停在 0.23.5。版本口径要齐。
- [ ] **提交并推送当前 `Ctrl+O` 修复。** 文档不能讲远端尚不存在的代码。
- [ ] **确认 GitHub CI 全绿。** 本地通过不等于三平台通过。
...
- [ ] 写一页 `CHANGELOG.md` 或给每个 Release 写清三条用户变化。
