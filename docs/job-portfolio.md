...
我用 C++23 独立做了一款终端 AI 编程工具。项目不只包了一层模型接口。我把三套流式协议归一成中立消息与事件，写了可扩展工具注册表、子代理、MCP/LSP 接入、上下文压缩与会话恢复。终端侧支持流式 Markdown、行内 Unicode 数学、块级盒式 LaTeX、可编辑输入、消息排队、diff 确认与工具明细折叠。底层进程管理分别适配 Windows Job Object 与 POSIX 进程组。当前产品代码约 4.75 万行，测试约 2.43 万行，本地全量 1244 个用例、5999 条断言通过，CI 覆盖 MSVC、GCC 与 Clang。
...
| 产品代码 | 188 个 C/C++ 文件，约 47,483 行 |
| 测试代码 | 81 个测试/夹具文件，约 24,324 行 |
| 自动测试 | 本地 Release 全量 1244 个用例、5999 条断言通过 |
| 提交记录 | 165 次提交，9 个版本标签 |
...
- 建立 CMake + doctest + GitHub Actions 交付链；本地 Release 全量 1244 个用例通过，CI 在 MSVC/GCC/Clang 三路编译测试，并按标签生成 Windows/Linux/macOS 发行包。
...
- 解决跨线程终端重画与跨平台进程回收问题；项目约 4.75 万行产品代码、2.43 万行测试，本地 1244 个用例通过，三平台 CI 与自动发布链齐备。
...
| “测试覆盖率很高” | “全量 1244 个用例通过”；没有 coverage 数据便不报百分比 |
...
正文、工具状态和可编辑输入并发写屏。项目现有约 4.75 万行产品代码、
2.43 万行测试，本地 1244 个用例通过，CI 覆盖 MSVC、GCC 与 Clang。
