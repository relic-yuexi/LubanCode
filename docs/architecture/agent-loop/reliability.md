...
> 2026-09-11 校注：本页保留旧路径说明。下文“主请求不重试”、SessionStore 收尾落盘、跳过坏行恢复等段落已不适用于当前默认 V3 路径。现版主循环已接入最多 6 次尝试的请求恢复环；当前失败处理、落盘闸门与恢复边界，请先读[失败之后怎么办](../../../interview/failure-and-recovery.md)，并沿源码核对。
