...
## 🧊 persistent 常驻 host:不做,依据在这里

process 插件每次调用起一只短命进程,"冷启动很贵"的担心要拿数字说话。
2026-08 实测(完整协议路径:spawn → import → stdin JSON → stdout JSON →
退出,不是光起解释器):

| 场景 | 环境 | 中位 | 最差 |
| --- | --- | --- | --- |
| 真实 runner.py(协议 v1 全链) | WSL Ubuntu, Python 3.14 | **14 ms** | 22 ms |
| 真实 runner.py(协议 v1 全链) | Windows 11, Python 3.13 | **39 ms** | 43 ms |
| 标准库中量 import(模拟常规依赖) | Windows 11 | **50 ms** | 86 ms |

对照:一次模型往返(网络 + 生成)以秒计;run_command 起个 git 也要几十
毫秒。冷启动占整次调用的 1%~4%,不是瓶颈。真到 numpy/pandas 那种
500ms+ 的重依赖,该先修的是"每次调用都 import 一遍科学计算栈"的插件
设计(拆成一次性预处理 + 轻调用,或作者自荐 venv 预热脚本),不是宿主
先造握手、心跳与重连——那是把 process 插件做成缩水 MCP,方向反了。

结论:v1 不做 persistent。哪天实测数字变了(比如某插件冷启动稳定超过
几百毫秒且拆不开),再立单做有界常驻 host;先有病,再吃药。
