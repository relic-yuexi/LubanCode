...
LubanCore SDK 是共用运行能力的主线，详见[LubanCore 解耦与交付单](../../todos/LubanCore与CLI分离_核心库独立成宿主底座设计.todo)。先抽中立装配和宿主 API，让 CLI、AppServer 与远端 Worker 共同消费。进程协议解决网络边界，不能替代可嵌入 SDK；网络连接、preview/full 投影与 CI 可独立并行，但不再各写一套工具装配。

先把本提案中的数据、寿命与协议合同收敛，再从集成分支开实现 PR。下面列远端各批，SDK-1/2/3 另在 Core 单验收。
...
| Remote-2 | Worker 通过共用 SDK/中立装配运行节点固定会话；不复制 CLI 工具栈 | 真读/写/命令/审批，v3 原地恢复，同项目同目录多 Session、各自操作账与停止范围 |
...
Remote-3 与界面流可在公共合同明确后分支并行。SDK 本地嵌入与共用装配同步推进，正式 Node Bridge 仍守 Core B/C 门。自动迁移、中央 Session Store、池化与强沙箱另排；不能以进程协议接通销掉 SDK 或完整集群验收。
