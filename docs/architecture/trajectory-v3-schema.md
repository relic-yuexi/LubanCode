...
原始捕获描述用 `preview_policy.policy=raw-capture-before-post-hook`，它还不是模型采用的预览。有效材料描述另存当次 `maxPreviewBytes`。富结果另有 `raw_payload.json`，保存原始文本、结构与媒体引用，不复制临时 wire base64。`combined` 与 `raw_payload` 在首次预览中共享字节预算。配额导致捕获中断时，两份描述都保留 `capture_complete=false` 和 `capture_reason=quota`。
