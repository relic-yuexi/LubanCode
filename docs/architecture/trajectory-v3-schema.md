...
P1 其余域已发行(工具操作账 `tool_action.*`、结果仓与预览 `result_store.*`、hook 事件账 `hooks.*`、subagent `subagent.*`、降档 `writer::ReduceToolPreviews`;T11 批再发行标题来源分家、环境快照、审批档、验证/迟到观察与容量压力/预算裁决,见 §2.1 T11 行);slash/skill/队列/估算/后台任务/todo 族留给后续棒次,字段仍按下列挂点冻结。
...
- **结果仓**(§4.16-4.17,已发行):目录 `sessions/<sessionId>/artifacts/`,`res-<六位号>.json` 为不可变描述(先临时文件、再落稳、再发布不可变名;POSIX rename 会静默覆盖,故显式查存在性)。描述文件 snake_case(`result_id`/`tool_call_id`/`attempt`/`result_kind`/`execution_event_ref`/`preview_policy`/`capture_limits`/`outputs[]`,outputs 项 `channel/ref/output_bytes/byte_count_kind/captured_bytes/capture_complete/capture_reason/encoding`);JSONL 内 result_ref 六键 camelCase(§3.1)。预览:完整渲染 ≤ 预算原样返回;超限先留说明预算 M、正文 B=预算-M 头尾均分(floor(B/2)/余),UTF-8 边界对齐,头尾段各重标 `[文件: path | 通道: channel]`,未展示正文文件点名 `not_shown`;清单超限先存 `output_index` artifact 再列容纳得下的路径 + `omitted_output_count`;`full_output`/`captured_output` 恒为数组,全部不完整时 `full_output=[]`;4 KiB 仍装不下必要来源记 `preview_unrepresentable`。T17(V3-ADD-03)起:预览给模型的路径为**绝对路径**(生产链按 session 目录拼;账上 `result_ref.path` 仍相对),截断的预览说明区追加 `retrieval_hint` 行——指路 `read_file` 按 offset/limit 分段读回原文,sha256 真值在 `tool.result.persisted` 六键。
...
| Telemetry(`src/telemetry/projector_v3.cpp`) | **已接 v3**(T07,2026-09-16):`ProjectV3LedgerFile` 投 v3 账,`gen_ai.usage.*` span 属性与 `lubancode.model.tokens` metric 吃 assistant usage owner(键集与 `UsageFromV3Owner` 同表,`tests/unit/telemetry/test_telemetry_projector_v3.cpp` 对表钉死);缺实报 `coverage=unknown` 不补 0;`model.usage.appended` 只作观察警告。2026-09-24 补验 service 集成面(V3-GAP-02 销项):流发现/行粒度游标/两代混投/重启续投不重复累计在 `tests/unit/telemetry/test_telemetry_service.cpp` 与 `tests/integration/telemetry/test_telemetry_otlp_export.cpp` 钉死 | 同上(assistant owner + appended 观察) | 已接(离线读面) |
...
| --- | --- | --- |
| 环境快照(`run.environment.captured`,§9.1:os/git/provider 快照) | **已发行**(T11-C,Session v3 旧设计清理单):`session.environment.captured`(snapshotRef/replayLevel/gaps/configRedacted,§2.1 T11 行)。取材件(`GatherGitStatus`/`BuildEnvironmentCapturePayload`)与 v2 同一台机器;没采集的场读取侧按缺件处理,不拿今天环境补昨天事实 | 已接:`CaptureEnvironment` v3 场落 v3 主账 |
| 手动/采纳标题(`control.title.changed`,/title 与自动采纳共用) | **已发行**(T11-A):`title.requested`/`title.extracted`(自动生成流,带真 `titleGenerationId`)、`session.title.applied`(source ∈ manual/local/generated/inherited;manual/local 不伪造 `titleGenerationId`——#52 一族的伪造号只作老档读取兼容,新写侧绝不再造)。自动生成的 prompt/assistant 由旁路桥落正式 message(purpose=session_title,§4.34 归首问主回合);迟到生成记 extracted 不落 applied;resume 折源场最后一枚 applied 为真实已采用标题,新场以 source=inherited 重申(inheritedFrom 五键指源事件) | 已接:`RecordTitleChanged`(manual)/`RecordLocalTitleApplied`/`RecordTitleRequested`/`RecordTitleExtracted`/`RecordGeneratedTitleApplied` |
| 审批档切换(`control.mode.changed`) | **已发行**(T11-B):`approval.mode.applied`(mode/source/policyVersion/oldMode)。档位变更与单次审批(channel.approval.*/确认门)分家;v3 场切档不再 memory-only。恢复重算:有效档 = 源场档与当前策略按宽严序较严者,`resume_recomputed` 事实带 oldMode,不静默提权 | 已接:`SessionManager::UpdateApprovalMode` v3 路写账;`OpenV3SessionLocked` 落 launch 基线;`ResumeAsNewV3Locked` 落重算事实 |
| 容量预检(`context.pressure.recorded`) | **已发行**(T11-E):`context.pressure.recorded`(phase/verdict/四项数字账/remainingTokens,§2.1 T11 行)。verdict 三分 reserve_clamped/exceeded_denied/max_tokens_degraded(降档经 `ContextPressure::Phase::PreflightDegraded` 只发边界记录器,v2 老账形状不动);禁携带累计用量——usage 唯一 owner 仍是 assistant message(§五) | 已接:轮桥 `OnContextPressure` v3 路 |
| 任务 turn 账(sent 边界数字,§11.1) | 无 kind。`model.request.sent` 本身照落 | 不落 |
| verification/恢复注记/迟到响应(`tool.verification.*`/`recovery.*`/迟到 mcp 响应) | **已发行**(T11-D):`tool.verification.recorded`(关联工具走信封 actionId,产物/事实按需)、`tool.verification.invalidated`(reason,append-only)、`tool.observation.late`(cause/jsonrpcRequestId;不改已提交终态)、`recovery.note.recorded`(note)。读面 `FoldVerificationFacts` 折 fresh——重复/迟到/变更后旧验证不封新目标,纯读不触发工具重做 | 已接:轮桥 trace 路(Verification/McpLateResponse/RecoveryMarker)+ `Begin/FinishVerification` v3 路 + undo 失效对账 |
