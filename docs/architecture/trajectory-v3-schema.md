...
| `purpose` | string | 必填,枚举:`conversation`/`compact`/`context_summary`/`session_title`/`capability`/`goal_evaluation`(capability 留给 §4.29 渐进披露;goal_evaluation 为 §4.67.6 验收内部回合:实际 system/user/assistant,不进 main 输入链,默认折叠) |
