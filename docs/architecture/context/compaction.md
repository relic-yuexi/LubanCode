...
| 题目 | 源码入口 | 关键测试 |
| --- | --- | --- |
| 单次与分层 compact | `src/agent/compact.cpp`、`compact.hpp` | `tests/test_compact.cpp` |
| token 与 hard trim | `src/agent/context.cpp`、`context.hpp` | `tests/test_context.cpp` |
| L1 结构压缩 | `src/agent/context_events.cpp` 等 context event 模块 | `tests/test_context_events.cpp` |
| L2 microcompact | `src/agent/microcompact.cpp`、`microcompact.hpp` | `tests/test_microcompact.cpp` |
| 中途压力与换史 | `src/agent/loop.cpp` | `tests/test_loop.cpp`、`test_request_prefix.cpp` |
| compact event 回放 | `src/agent/session_store.cpp` | `tests/test_session_store.cpp` |
