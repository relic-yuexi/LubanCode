...
当前实现不在同一 step 内自动重试，故模型请求数正好等于 step 数。日后若添请求重试，另记 `attempt`，那时才有 `attempt_count >= step_count`。不可把重试也算成新 step。失败分类、`max_tokens` continuation、ESC 收口与 session 修补，见 [Agent Loop、重试与恢复深挖](agent-loop/reliability.md)。
...
上下文涨到 80% 左右时，主循环会先调用 compact 模型，把老历史摘要成一条 archive（六栏存档 + 末尾 JSON manifest，活动待办漏一项就拒收、历史不动），再按 token 预算保留最近热区。压缩模型自己的窗口单独算预算，装不下就明确拒绝，不静默截史。若单条工具结果巨肥（超过估算窗口的四分之一），保命索会把它的尾部截短（token 轴口径），触发时打醒目告警；模型不在目录、窗口查不到时按 128k 兜底窗口评估，不裸奔。
