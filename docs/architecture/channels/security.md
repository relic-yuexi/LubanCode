...
| sidecar 越权 | 不走 shell 字符串只传 argv；state_dir 单账号隔离；crash/超时/坏帧杀整棵进程树；Package 未信任不执行。首版 sidecar 仍拿当前用户 OS 权限，文档不虚称强沙箱。QQ（Q1）、企微智能机器人（W1，长连接 BotID+Secret 走 `app_id`/secret 既有字段与密钥规矩）与飞书（F1）已定案进程内直连：spawn/环境继承/进程树三面不适用，凭据不出宿主进程；本行防线留给未来受管子进程渠道 |
