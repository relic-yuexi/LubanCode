...
[文档首页](../README.md) · [开发指南](build-and-release.md) · [终端交互](../features/terminal/README.md) · [文档规范](documentation.md)
...
`deepseek_e2e` 是现有示例：没设 `DEEPSEEK_API_KEY` 时自行跳过，不进 ctest。PTC 真模型探针与手测矩阵见 [PTC 手册](../features/tools/ptc.md)。
...
同时记录每轮模型请求数。三条用户消息不等于三次请求；工具往返会增加 step。术语按[命名与计数规范](naming.md)。
