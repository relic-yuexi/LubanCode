...
`lubancode im` 的前台会随每轮输出 session/turn、模型步骤、工具请求与执行、审批等待与结果、回复入队与 QQ 回执。工具命令、路径及结果只显示有界摘要，并调用现有密钥脱敏器；不展开整份文件正文或图片字节。终端可能出现工作路径和业务摘要，分享日志前仍应检查。

用量按供应商回报显示输入、输出、缓存读写、读缓存占输入比例与 API 耗时。未报告写“未报告”，明确报告为零才显示零。本地前缀 epoch、变化原因单列，不能据此认定远端命中缓存。

context 同时显示请求前估算、输出预留、运行窗口及占用比例；收到用量后再显示本次实际输入占窗口比例。整轮累计消耗另列，不拿它冒充当前上下文。窗口使用运行配置，未配置则由引擎兜底。达到压力阈值不代表已压缩；工具结果有损截断、超窗错误按实际事件报告。模型结束也不等于回复送达，须看后续 QQ 回执。这些信息显示在启动 Gateway 的终端，不是 QQ 内仪表盘。
