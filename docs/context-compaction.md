...
[文档首页](README.md) · [会话与上下文](sessions-and-context.md) · [Query 数据流](query-data-flow.md) · [项目记忆流程](memory-system-flow.md)

这页只讲一件事：长会话怎样收短，又怎样守住尚未做完的活。配置字段与命令语法见[配置手册](configuration.md)和[命令参考](commands.md)。项目记忆另走一套链，见[项目记忆流程](memory-system-flow.md)。
