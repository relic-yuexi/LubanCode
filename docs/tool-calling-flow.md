...
[文档首页](README.md) · [工具参考](tools.md) · [Query 数据流](query-data-flow.md) · [Hooks 流程](hooks-flow.md) · [PTC 手册](ptc.md)

这页讲一枚工具调用怎样从模型手里落到本机，又怎样把结果送回模型。工具名、参数与输出上限见[工具参考](tools.md)；三种 wire 的报文差异见[Query 数据流](query-data-flow.md)。
...
长结果进请求前还会过上下文视图层：重复只读结果可折成引用，超长结果可换 artifact 预览。执行与 session 真账不受影响。详见[上下文压缩机制](context-compaction.md)。
