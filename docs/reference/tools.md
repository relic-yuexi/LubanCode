...
| `agent_watch` | 挂了 `agent` 的会话 | 只读台账并可等待 | 否 |
...
只读的子任务监督窗口。主代理可看整棵会话树（独享 `diagnostic` 档），每只会派工的子代理拿到绑定自己身份的窄实例，只看自己的直接子任务——越 lineage 一律稳定拒绝。

| 参数 | 必填 | 说明 |
| --- | --- | --- |
| `task_ids` | 否 | 任务号数组，最多 16 只；不填按 lineage 取缺省集（main 看全部运行中根任务，子代理看直接孩子）。 |
| `after_revision` | 否 | 上次返回的 `revision`；相同且 `wait_ms>0` 时睡到任务变化或超时。 |
| `wait_ms` | 否 | 最多等 30 秒；用户介入、取消、会话收场都提前唤醒。 |
| `include` | 否 | `summary`（缺省）/`events`（另带至多 50 枚结构化事件）/`diagnostic`（仅 main）。 |

等待走条件变量，宿主零忙轮询；输出只含状态、阶段、健康、静默龄、重试数与稳定错误码，不带思考、正文、Secret 与完整工具参数。故障检测不靠它——宿主里的 `AgentSupervisor` 一直在跑；它只回答"现在怎么样、变了没"。
