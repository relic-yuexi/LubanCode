...
本文是冻结合同。工具怎么裁、功能怎么关、能力快照记什么、一场会话的运行材料活多久、受理真账写到哪一档,一概照此办理。设计出处见 对应设计单(P0 阶段冻结)。
...
| `components.skills` | 来源声明式 schema(应用Worker接入单 §六,additive):`required`/`optional` 名单(canonical ID)+ 可选 `sourceDir`(材料根 `skills/` 内的相对子目录,`/` 分段、不收绝对路径/`..`/反斜杠)。声明即允许清单——名单外的根内技能不进本场;required 缺件/坏格式报 `skill_missing` 整场明拒(人话带扫描警告),optional 缺件跳过出诊断;同名 required/optional 两现或名单内重复是配置错误。声明须 `features` 放行 `skills` 且 `tools` 点名 `skill`(三面同进同退);thread/start 回执带冻结清单 `skills`(name/requirement/status/requiresTools/missingTools,additive) |
...
依赖解释是校验的一部分,不是可选项:`tools.allow` 里的 `mcp:<server>:<tool>` 名字要求 `components.mcpServers` 含 `<server>` 且 `features` 放行 `mcp`;裸名 `skill` 要求 `features` 放行 `skills`;`plugin__<id>__<tool>` 名字要求 `components.plugins` 含 `<id>` 且 `features` 放行 `plugins`;`components.skills` 声明获准名要求 `features` 放行 `skills` 且 `tools.allow`(deny 折算后)含 `skill`;`exposure.default: deferred` 要求发现器依赖(tool_search/tool_invoke)未被禁,零工具面(`none` 或 `only`+空 allow)配 `deferred` 是配置错误——没有可延迟暴露的东西。解释不全的档不许采用。
...
外部应用常驻、每项独立研究起一只 Worker 时,LubanCode 进程的材料来源与状态落点由三枚环境变量钉死。设计出处 对应设计单 §四。
...
状态落点:workspaces/会话账、workflow-runs、browser-artifacts、package-trust/package-state/hook-trust/plugin-trust、hooks-outbox、plugin-data/package-data、package-store、cache、logs、memory(含 memory-jobs)、**channels 渠道账号状态树**(`<状态根>/channels`:ingress 账、account-status.json、sessions 映射、work 账、pairing、账号锁、qq spool——单一口 `channel::DefaultChannelsStateRoot()`)、**gateway profile 树**(`<状态根>/gateway`:profile 锁/控制命令/boot 历史/日志/automation 账/delivery(outbox/replies/published)——单一口 `gateway::DefaultGatewayRoot()`)全部落**数据根**;参数根可挂只读。个人 CLI(未设应用根)一切落 `~/.lubancode`,行为与从前逐字节一致(AW-01)。归属裁定(P1 遗留销账):`ptc_profiles.json`(PTC 用户偏好画像)归材料不归状态,留**参数根**——应用根语义=`<LUBANCODE_HOME>/ptc_profiles.json`,个人布局=`~/.lubancode/ptc_profiles.json` 原样;旧位置文件照读,不暗迁移、不暗删(生产面只读,写档归人工探针流程,参数根挂只读不破坏)。归属裁定(P1 收尾):`gateway.json` 是用户手写档,但与锁/账/日志同住一棵 profile 树,整树随状态根走、不拆双根——应用根语义下写 gateway.json 须写到数据根的 profile 目录内;渠道侧受保护路径闸(tool_guard)的护面同锚状态根(全局 config.json 仍锚参数根)。
...
| SkillTool/技能材料装载 | **已落(P2,2026-09-14)**——features.skills 放行且 tools 面点名 `skill` 才装配:显式单根(材料根 skills/)扫描,清单段/工具/预装正文三面同进同退;skills.preload 缺名整场明拒;不搬终端五层合并,装技能不授予任何执行工具。**本批补齐(§六尾款)**:①漂移校验——扫描记 SKILL.md 全文指纹(`SkillMeta::content_hash`),按需读时对照,同场中途改文件拒读(`skill.drifted`,修改版正文不进上下文);②受控资源读取——skill 工具可选 `path` 参数只读技能目录内相对材料(绝对路径/`..`/UNC/盘符/外链 URL 明拒,链接解析后越出技能目录拒,1MiB 帽;外链不自动 fetch,明示走数据源流程);③依赖声明消费——frontmatter `requires-tools` 只作声明,headless 装配递冻结工具面,缺获准执行工具加载即回 `capability_unavailable`(终端路全量注册+运行时审批,不同形,缺省不执法);④components.skills 来源声明(见 §3 字段表)——required 拒启/optional 诊断/名单外不进/冻结清单回执 | `AssembleSession` 步骤 2.5(src/app_server/session_assembly.cpp)+`tools::ScanSkillsDir(Reported)`/`SkillTool`(src/tools/skill_tool.cpp);组合 `ComposeHarnessSystemPrompt`;测试 tests/unit/config/test_skills.cpp+tests/unit/app_server/test_session_assembly.cpp |
| MCP 子进程凭据分离 | **已落(本批,§7.1)**——headless 装配起 MCP 子进程改 `EnvMode::Replace`:环境=base 集(PATH/系统基件/临时目录,与 plugin_process 同一张合同)+ 部署配置 `mcpServers.env` 注入(工具自己的凭据),宿主其余环境(模型 API key 一类)一概不递;受控故障场景夹具(auth_gate -32001/rate_limited -32002/slow/empty/env_probe)与逐层状态(server 拒绝归 `mcp.server_error` 带 `details.jsonrpcCode`、超时 `mcp.timeout`、取消 `mcp.cancelled`、空结果=成功)已验 | `ComposeMcpChildEnv`(src/app_server/session_assembly.cpp)、`mcp::Client::SendRequestAndWait` JSON-RPC code 带出(src/mcp/client.cpp)、夹具 tests/fixtures/mcp_test_server.py;测试 tests/unit/app_server/test_session_assembly.cpp(env 口)+ tests/integration/protocols/test_mcp_client.cpp(场景)+ tests/integration/app_server/test_app_server_agent_skills.cpp(整链 env 分离) |
| 提示组合可追溯记录 | **已落(本批,§五 134)**——部署档组合路(agent_plan)在 v3 场开场落 `prompt.composition.applied` 事实行:组合次序(segments.order 从 0 连续)、各段渲染正文 SHA-256、来源层与磁盘路径、最终快照 ID(拼装正文全文 SHA-256);业务正文哪段来自哪层逐段在账,业务文本伪装不了宿主权限。v2 场无此行(不回填旧格式);落账失败只记诊断不拦建场。§五 133 同批钉死:托管模式(部署档在场)拒绝 `--system-prompt`/`LUBANCODE_SYSTEM_PROMPT_FILE` 覆写源(部署档 schema 未开覆写通道,冲突未定义次序即拒启;普通 CLI 语义另保留) | v3 kind `prompt.composition.applied`(src/trajectory/v3/envelope.hpp+schema3.cpp 载荷合同)、`HarnessPromptResult::snapshot_id`(src/app_server/agent_wiring.cpp)、落账 `Server::HandleThreadStart`(src/app_server/server.cpp)、覆写门 `RunAppServerMode`(src/app/cli_app.cpp);测试 tests/unit/trajectory_v3/test_v3_prompt_composition.cpp+ tests/integration/app_server/test_app_server_agent_skills.cpp(整链) |
...
| gateway/channels 状态根接数据根 | **已落(P1 收尾,2026-09-14)**——gateway run 装配段(cli_app.cpp)的 workspaces 根、workspace 身份裁决 home 止步、gateway profile 树、渠道账号状态树全部改走 `config::StateRootDir()`(应用根语义=数据根;个人布局=`~/.lubancode` 原样,行为逐字节不变);两棵树各有单一口 `gateway::DefaultGatewayRoot()`(`<状态根>/gateway`)与 `channel::DefaultChannelsStateRoot()`(`<状态根>/channels`),`gateway status` 渠道栏、`gateway run` 装配、受保护路径闸三处消费同锚;QQ Q1/Q2 落的状态件(ingress 账/account-status/sessions 映射/work 账/reply outbox/qq spool/账号锁)建账时即按注入根落位,随装配切到数据根 | `channel::DefaultChannelsStateRoot`(src/channel/manager.cpp)、`gateway::DefaultGatewayRoot`(src/gateway/profile.cpp)、RunGateway 装配段(src/app/cli_app.cpp)、`DefaultGatewayRoot`(src/cli/gateway_command.cpp)、`DefaultChannelProtectedPaths`(src/channel/tool_guard.cpp);测试 tests/unit/config/test_runtime_paths.cpp(渠道/gateway 状态根三态+护面锚定) |
...
| 插件执行约束(装配路):取消链/资源墙/v3 执行账 | **已落(尾款,2026-09-14)**——三项在 app-server 装配路(AssembleSession→registry→adapter)下钉 CI 用例:取消链(`ToolExecutionContext.cancel` 贯通 instruction hook,RunOneTool 同款递进口,旗置位即掐零产出);资源墙(指令帽 200M/内存帽 256MiB 按 PureDefault 真预算在装配路触发,稳定错误 `plugin.lua_error`+分型正文);v3 执行账(整回合生产装配工厂路:插件工具成功落 `tool.execution.pending/started/finished` 同 actionId、失败落 `tool.execution.failed`(error_code=`plugin.lua_error`),`model.request.prepared` 带 provider 真值)。**未接面如实**:墙钟墙在插件工具路结构性未设(PureDefault wall_budget=0,`LuaHostState::Call` 不灌 wall_deadline——墙钟只在 hook 路生效),记 §7.2 未验边界 | 测试 tests/unit/app_server/test_plugin_assembly.cpp(取消链/指令墙/内存墙/v3 执行账四用例);机制层底座 src/runtime/plugin_lua_host.cpp(GuardHook)、src/tools/lua_tool.cpp(LuaGuard) |
| 模型连接启动快照(单 Worker 冻结)+ 脱敏 effective-config 诊断 | **已落(尾款,2026-09-14)**——生产入口启动时从 Config/ConfigSources 冻结一份连接快照(wire 规范名/model/provider/脱敏端点/密钥引用/配置版本/逐字段来源/角色路由三态):thread/started 回执 additive `connection` 字段逐场同一份(单 Worker 连接冻结,运行期改配置只影响新 Worker);v3 `model.request.prepared` 载荷带 `connection` 块(endpoint/secretRef/configVersion)与 provider 真值;启动时 stderr 打脱敏 effective-config(根路径/来源清单/取值来源/部署档指纹/功能状态,零密钥)。红线:密钥正文与密钥内容哈希不进任何一处;端点只留 scheme://host[:port];配置版本是非敏感事实的 sha256(换钥匙不动版本)。**缺口如实**:app-server 装配未把 config.model 折进 `AgentProfile.request.model`(`ServerOptions::session_model` 至今无消费方,请求 model 恒空)——归"AppServer模型连接解耦"owner 单接线;凭据经 child 专属 env 交付的进程级证据在 P4 e2e(AW-18) | src/app_server/connection_snapshot.{hpp,cpp};入口冻结 RunAppServerMode(src/app/cli_app.cpp);thread/started 与回合桥 Identity(src/app_server/server.cpp、src/runtime/trajectory_session.cpp);测试 tests/unit/app_server/test_connection_snapshot.cpp + test_plugin_assembly.cpp(v3 请求账 connection 块) |

错误码归属:路径/来源类配置错误在启动门以 stderr 人话+退出码 1 拒启(与 CLI 既有风格一致),不另立协议错误码;协议面"点名未接线组件报 `component_unavailable`"自 P2 生效(装配失败经 thread/start 错误信封带 `data.code`,additive 字段)——P5 起 v2 embedded-lua 真装载,此码收窄到"runtime kind 未接线"(点名 process/native 件),另增三枚插件装载失败码:`plugin_missing`(点名件不在发现账,含 manifest 坏被扫描剔除)、`plugin_untrusted`(信任账不过:未信任/disable/内容指纹算不出)、`plugin_load_failed`(Lua 挂载坏:entry 读不到/编译坏/handler 对账不过),同一错误信封带出;本批再增 `skill_missing`(components.skills 声明的 required 技能缺件/坏格式,整场明拒,同一错误信封)。"`capability_unavailable`"(本单 §六冻结)已随本批 Skill 依赖声明消费生效——skill 工具加载声明了 `requires-tools` 而本场工具面缺件时,工具结果回该码(非整场拒,按需加载时逐次判定)。
