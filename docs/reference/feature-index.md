...
| Slash 候选 | `/` 开头实时提示；Tab 补全；按下方向键可进入直选菜单。空闲与流式排队输入框同样生效，仅交互终端。 |
...
详见[终端交互](../features/terminal/README.md)与[命令参考](commands.md)。
...
详见[会话与上下文](../features/sessions/README.md)和[项目记忆](../architecture/memory/design.md)。
...
详见[扩展指南](../features/extensions/README.md)。程序化工具调用另见 [PTC 手册](../features/tools/ptc.md)。
...
CI 在 Windows/MSVC、Ubuntu/GCC、macOS/Clang 三路编译并测试。`v*` 标签从干净 runner 重编、打包并创建 Release。构建、测试与发包细节见[架构说明](../architecture/README.md#15-测试与交付)。
