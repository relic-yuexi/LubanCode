...
[文档首页](README.md) · [架构说明](architecture.md) · [测试指南](testing-guide.md) · [文档规范](documentation-standard.md)
...
依赖方向与一轮请求见[架构说明](architecture.md)。不确定改哪层时，先问一句：这段逻辑在没有终端、没有某家协议、没有某件工具时还成立吗？答案能帮你把它落回合适边界。
...
5. 改用户可见行为，按[文档规范](documentation-standard.md)同步页面。
...
详见 [Query 数据流](query-data-flow.md)。
...
详见 [终端交互](terminal-ui.md) 与 [测试指南](testing-guide.md)。

## 10. 发行

版本号同时在 `CMakeLists.txt` 与 `src/app/version.hpp`。两处须一致。用户变化写 `CHANGELOG.md`。

推送 `v*` 标签后，release workflow 在 Windows、Linux、macOS 重编，打包程序、双语 README、`docs/`、`skills/` 与安装脚本，再创建 GitHub Release。不要拿本机 `build/` 产物手工塞进发行包。
...
| 只改 Skill 却没同步 | 构建 `lubancode_official_skills` 或主默认目标 |
| Debug/Release 路径找错 | 多配置生成器在配置名子目录下放 exe |
| CI 与本机下载路线不同 | CI 关了 codeload；行为应同，依赖来源不同 |

更长的症状树见[排错手册](troubleshooting.md)。
