...
[文档首页](README.md) · [命令与按键](commands.md) · [配置手册](configuration.md) · [测试指南](testing-guide.md)
...
配置优先级和默认值见[配置手册](configuration.md)。
...
详见[会话与上下文](sessions-and-context.md)与[测试指南](testing-guide.md)。
...
`/memory why` 应优先给召回 trace，不要靠猜关键词。详见[项目记忆](memory-system-design.md)。
...
详见[开发指南](development-guide.md)与[测试指南](testing-guide.md)。
