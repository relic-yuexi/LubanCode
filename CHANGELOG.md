...
## [v0.26.174] - 2026-09-02

- **思考活动条不再每秒闪五次。** 此前 200ms 心跳逐字轮换高亮"思考中"，每拍都改帧指纹引发重画，配合终端同步输出能力误判，光标在活动条与输入框之间反复跳。现活动行只在秒数、阶段或中断态变化时重画；逐字扫光删除，留静态蓝点与秒钟。
- **DEC 2026 同步输出改为主动问。** 终端能力拆成普通 VT / 原地重画 / 同步输出三档各自探测（发 DECRQM 询问、应答才算认得）——不再拿 vt_enabled 顶替同步能力。不认 2026 的终端走明确降级路（Windows 退原生控制台 API，不搬实体光标），误判永远落在安全侧；帧账新增 paint 档位诊断。真终端录证（Windows Terminal/conhost/VS Code 各十秒静置与边输入边思考）留白待真机。

## [v0.26.173] - 2026-09-02

- **错误形态有了账本。** 实战撞见的中转站错误（`upstream_error` 一类）与 401 等确定性错，从轨迹实录入册 `error_shapes.json`——每条记 wire、HTTP、错误码、现行重试判定与首见出处，采集脚本扫全部会话轨迹幂等回填，`--check` 可当 CI 门。往后每撞见新错误形态，账本为证、矩阵跟上，"该不该重试"不再拍脑袋。
- **实录形态进了测试矩阵。** 采样实录喂进恢复环用例——200+`upstream_error` 与 500 两路用满预算后，失败文案如实带"已自动重试 5 次"与含退避总时长；401 零重试用例照旧钉死。

## [v0.26.172] - 2026-09-02

- **派工前先查单，修完的单不再白跑。** 任务书指向 todos 单子时，若该单在当前代码上已标"已实现/已销"且无未勾批次，派工口在建隔离房、请后端之前稳定拒绝并提示——此前会默默开工，在旧树上把修完的活重做一遍（实战实录：739 秒、2.26M tokens 全部沉没）。
- **隔离房基线全路径冻结。** 派工口早段冻结调用者 HEAD，建房口二次核对——期间 checkout 漂移、嵌套错拿宿主目录、传错快照，都在 `git worktree add` 之前阻断。12 只新用例钉死嵌套基线、漂移阻断、查单拦截。

## [v0.26.171] - 2026-09-02

- **构建命令不再 120 秒挨杀。** `run_command` 默认超时按命令分档：cmake/msbuild/ninja/ctest/gradle/cargo 一类构建与测试命令默认放宽到 15 分钟，其余维持 120 秒；显式传 `timeout_ms` 永远优先。超时报错带三条引路——加时限重跑、转后台、查依赖下载是否卡网——模型撞墙后拐弯，不再原样重跑烧 token。
- **该重试的错真重试了。** 此前上游回 500 不在白名单、HTTP 200 带错误体恢复环根本看不见——中转站一抖，子代理一发毙命（实战三只阵亡实录）。现 5xx 全家认齐，`upstream_error`/`server_error`/`overloaded` 一类六枚错误码可重试，四条 wire 的错误码保真传进恢复环；401/400 等确定性错照旧零重试。

## [v0.26.170] - 2026-09-02

- **长任务的窗口预检进轨迹了。** 上下文压力判定（估入/预留/余量三项账、应急收窄放行与稳定拒绝两支）以 `context.pressure.recorded` 事件落账——此前只在面板与日志里闪一下就没了，现在轨迹可查、insights 可投影。派工隔离单验收门的最后一格补完。本笔是隔离房遗作：子代理写完未及编译即被上游错误击杀，主控验证构建与全量测试后收录。

## [v0.26.169] - 2026-09-02

- **通名环境变量退役了。** `ANTHROPIC_*`/`OPENAI_*` 一概不再读取——环境变量只认自家命名空间 `LUBAN_*`（`LUBAN_BASE_URL`/`LUBAN_MODEL`/`LUBAN_API_KEY`，既有 `LUBANCODE_*` 同族保留）。此前 Claude Code 这类工具往环境里塞的通名变量，会悄悄改走 wire、解绑 provider、废掉配置里贴好的钥匙（401 无头案）——现在四个通名齐注的污染环境下，inline 钥匙照常生效，零警告零干扰。
- **`--config` 的来源标注换了口径**：环境来源明示 `[环境变量(LUBAN_*)]`，一眼分清是自家变量在生效还是配置文件在兜底。provider 的 `key_env` 显式指定照旧可用（向导对指向通名的仍给撞车提示）。干净砍不走兼容窗——通名喂配置的用户改用 `LUBAN_*`。
- **本版两段接力写成**——第三场实战中途被杀（改动未提交），续场进同一 worktree 摸清前任进度、补齐收尾：断点恢复的实战首秀。361/361 全绿。

## [v0.26.168] - 2026-09-02

- **缓存账睁眼了。** openai 线（responses/chat）此前不挖 usage 回包里的 `cached_tokens`，逐请求轨迹的缓存列全零、汇总行的命中率来历不明——两头对不上账。现逐请求 `cache_read_tokens/cache_creation_tokens` 填真值，`[tokens]` 汇总与逐条之和同源同算法，真机对账分毫不差。provider 没报缓存就如实标"未报"（新增 `cache_reported_by_provider`），旧轨迹零值不冒充明报。
- **缓存命中可按段看。** compact 必断前缀是结构性必然——现在 insights 按 `cache_epoch` 分段统计，compact 后算第二段整体，段内各看各的命中，不再一个总池混算。
- **本版由 LubanCode 自己写成**——实战第四场：一小时零四分自走（361 册全绿、真机自验轨迹对账），与前几场不同处在于它跑完的轨迹本身就是验收材料。

## [v0.26.167] - 2026-09-02

- **顶层通名环境变量退役。** `ANTHROPIC_*`、`OPENAI_*` 不再被隐式读取，外部工具注入的同名变量不会再改写连接或误触发 provider `environment_unbound`；改用 `LUBAN_BASE_URL` / `LUBAN_MODEL` / `LUBAN_API_KEY`，既有 `LUBANCODE_*` 保留且同设时 `LUBAN_*` 优先。provider 显式配置的 `key_env` 仍可指向任意变量名（包括通名）。

- **断流重试加厚到五回，等得也久了。** 此前首发加两回重试、最长等 2 秒——碰上中转站重启或上游限流（恢复窗口常在半分钟开外），3 秒烧完就报死。现改为总尝试 6 次（首发+5 回重试），退避五档指数带抖动：1~2s → 4~8s → 15~30s → 30~60s → 60~120s，单次封顶 2 分钟。401 一类确定性错误照旧零重试，重试更久也没用的病不归它治。
- **退避中不再被监督器误喊停滞。** 长退避等待期间健康态归 Recovering（"重连 N/5"——上限联动真常量，不再写死）；失败文案带真实重试次数与含退避的总时长，中英成对。
- **本版由 LubanCode 自己写成**——实战第二场：39 分半自走（读单、写码、构建、361 册全绿、落 commit），缓存命中 68%，细账头一回落进轨迹会话（逐请求 usage 随时可查）。

## [v0.26.166] - 2026-09-02

- **API 钥匙不再撞车撞得莫名其妙。** 解析规矩改为：配置里的 inline api_key 是底座，`key_env` 指的环境变量存在才覆盖；两把都在又不一样，启动一行警告点明变量名与用的是哪把（打码显示），不再静默选边。
- **`--config` 说人话了。** api_key 一行明示来路六种情形——"来自环境变量某某（压过配置里的 inline）""来自配置 inline（key_env 指的变量未设）"——钥匙哪来的、用的哪把，一眼可见。
- **缺钥匙的报错带路。** 提示里点名当前 provider 的 `key_env` 变量名与 inline 状态，知道该去设哪个变量还是改用 inline；配置文档与首次配置向导写明优先级与通名（ANTHROPIC_*/OPENAI_*）撞车风险。

## [v0.26.165] - 2026-09-02

- **单发会话进账本了。** `lubancode "任务"` 这类 one_shot 从此过轨迹账本：run_kind 单列（默认不进训练集，配置可改），逐请求 usage 落账——输入/缓存读/缓存写/输出/推理五列分列。此前一场 108 请求的重活儿在账本上查无此场，只有退出前一行总账；现在细账随时可查，缓存命中低不高一翻便知。
- **单发场可管理可审计。** `/sessions` 列出单发场（标注"审计可读，不续聊"），归档/删除照管；Token 账本、/prompt audit、/insights 全管线天然纳入——最烧钱的用法不再是糊涂账。
- **中途死活有档。** 401 早死、进程被杀：已发出的请求、失败原因、跑到哪一步，账里如实可查（WAL 语义验证过）——重跑前先看档，不再两眼一抹黑。退出前 `[tokens]` 汇总行照打，多一行"细账见轨迹"指路。

## [v0.26.164] - 2026-09-02

- **隔离房不再从旧地基起墙。** 派子代理开 worktree 的基线改为冻结的调用者当前 HEAD（此前从 origin/main 解析，本地领先远端时孩子会在旧代码上干活）；建后对账 base_ref/base_commit/actual_head 三值，不符当场拆房报错。调用者树脏会附言明示，未推提交不算孩子的活。
- **派工前先验能力，不再跑到执行口才翻脸。** 当前入口没有后台后端时，schema 里就不亮后台选项、派工前稳定拒绝（错误码+入口+可用模式+改法四件套）——不再收了 `run_in_background=true` 跑到注册后才报错。
- **长任务不再被 128k 输出预留掐死。** 查明来路是模型目录能力上限被原样继承；现对子任务收窄为窗口八分之一（8k~32k 限幅），预检最终闸再兜一层应急预留——装不下时收窄本请求放行并注入收尾交代（已跑命令/关键结果/未提交改动），同任务可续跑。**已提交现场进"待主控复核"态，不再回传完就删树**——复核命令与重挂法随结果带回。全新 worktree 的 rg 也有了全机分期兜底，不用再手工拷。
...
- **Agent Dock 会说自己怎么了。** 每行添一小截：等首字节多久、工具跑了多久、第几次重连；查看态多一条"监督"折叠行——最后传输、最后完整提交、当前 attempt、最近错误码、下一动作与截止。恢复成功、重试用尽、工具结果不明各弹一条通知，按任务去重不刷屏。
- **健康账接了只读钩子与指标。** `AgentHealthChanged` 后台安全队列送达，钩子慢坏不卡监督器；六枚低基数指标（基数闸有测试钉死），快照可直接走现有遥测编码口。跨平台进程测试与真机长跑留档未做。

## [v0.26.159] - 2026-09-02

- **按需调工具这单，验收收官。** P0~P4 红线内逐批过堂：三拍前缀合同、四 wire 代理回放、条件工具 hash 守恒、Claude 原生链无损，各有测试册与读码证据钉着；362/363 绿（余一为并发超时假红，单跑即绿）。真机 cache 账那栏无钥匙不冒充——对照脚本在位，配钥匙即可跑。
- **compact 不丢工具账，补了册钉着。** 此前"发现过的工具引用在换史后照常可用"只是实现上的不作为，最怕没人看着——现在有专门测试册守门：发现→换史→旧引用照常解析、照常走正门调。
- **文档补上 `tool_invoke`。** 可用性总表原先缺这一行，tool_search 小节只写了旧行为；现在三档差异（legacy/代理/原生）与调用入口都写齐了。

## [v0.26.158] - 2026-09-02

- **Windows 装得上也卸得净。** Tauri 壳的 NSIS 安装包真出货：静默装、验文件、起进程、卸干净四步真机走通，卸后目录、注册表、快捷方式零残留。壳不捆内核——连用户已装的，版本不合只摆提示不拦截。
- **壳的流水线上线。** GitHub Actions 两条腿：Windows 出 NSIS（产物哈希记账），ubuntu 出 APK（runner 自带 JDK17+SDK，gradle 钉版本跑 assembleDebug）；两册壳冒烟挂进 CI 必跑。首跑未验——推上去见分晓。
- **壳与内核的兼容有了声明。** 前端握手对表 PROTOCOL_COMPAT（闭区间），不合转提示条摆人话；三只前端（浏览器页/Tauri/Android）同一份口径，没验过的不冒充兼容。Android 签名占位接线，无证书如实告知，keytool 命令进了文档。

## [v0.26.157] - 2026-09-01

- **断流自己救自己。** 模型请求碰上网络错、超时、网关 5xx，自动按抖动阶梯重试三次（250ms→1s→2s，可打断）——半截输出不留账，重发拿整份；重试烧尽才报错，报错文案带"已自动重试 N 次"。子代理卡死有四本时钟看着：先提醒宿主自救，三轮无进展才收口，绝不冒充用户按停。
- **派工烧不穿预算了。** 子代理的请求额度从"每轮各领各的"改成"任务级一本账"：配 2 次就是 2 次，续投、钩子续跑、孩子回流全从同一本账扣，第四枚请求压根不发。自定义 Agent 与 Workflow 节点同吃这本账。
- **轻工具不再为延迟倒贴。** 延迟加载的工具有了 token 本金门槛（1500）：工具定义本身就没几个 token 的，全量常驻反而更省——门槛由两组实测数据钉出来的，可调可关。

## [v0.26.156] - 2026-09-01

- **敲 /insights 看全局账。** 跨会话、跨工作区的总报告：时间窗（日/周/全量）、五态场次覆盖（active/incomplete/corrupt 各多少）、摩擦点与功能信号汇总，还能出自包含 HTML 存档。账从轨迹投影而来，flag 没开的场次如实标缺，不硬凑。
- **工作区有了唯一的身份。** 同一仓库不管从根目录、子目录还是 linked worktree 进，都认成同一个工作区（此前两套身份算法各算各的，子目录启动会把账记裂）；独立 clone 不混账，嵌套仓取最近者。`/doctor` 能对账 manifest，改过手脚当场报 `identity.key_mismatch`。
- **渠道消息会自己找家了。** 消息按账号/会话/话题路由进各自的 headless 会话；须确认的工具在渠道里没有审批口就干脆拒绝（不冒充用户点头）；记忆默认隔离，只有白名单里的人说话才碰记忆。
- **search 换心收官。** 旧自研内核连同差分门一并清仓（净删 815 行），回滚策略入档；随包 rg 的 Linux 资产经查实为 musl 静态构建，按实测背书接受并记 watch。

## [v0.26.155] - 2026-09-01

- **本地遥测有了状态面。** 配置里开了 `features.telemetry` 的会话可敲 `/telemetry` 或 `/doctor telemetry` 看投影服务、暂存池与游标的健康账——纯本地只读，不发一字节出网；没开的照旧，一行不注入。断电重启后暂存批自愈：截断的半批隔离，完整批补齐入账。

## [v0.26.154] - 2026-09-01

- **会话标题默认就用模型起名。** 首问一出，后台并行发一枚短请求把标题精炼成十几个字；没单独配便宜模型就照用当前模型（新裁决：cheap/lao 未配置默认走 normal，不再因"回落"放弃）。失败保留本地截断标题，不重试不烧第二次。
- **敲 /prompt audit 看 prompt 账。** 十二条静态规则查系统提示的重复段、超长工具描述、schema 硬伤；运行时逐请求对账快照、工具表抖动、前缀断点。只摆事实不带正文，语义类问题标"疑似"不扣帽子。
- **Claude 原生工具搜索接上了（需模型声明）。** 目录声明支持 server tool search 的模型（fable-5、opus-5 等）可走官方 `defer_loading` 原生路：延迟工具定义照发全文只改加载方式，原生工具块无损解析存档；未声明的模型照旧，不按厂名猜。
- **多渠道消息有了账号管理面。** `/channels` 与 `/channel show|doctor|start|stop`：账号状态机、入站消息耐久账、断线重开不丢信。另修技能同名冲突提示重复播报两遍的小病。
...
- **条件工具不再断缓存。** goal/loop 的专用工具定义改为会话内常驻，开关挪进回合内提示——翻多少轮，缓存前缀纹丝不动；不在相应轮次硬调，照拦不误。
- **轨迹能导训练集了。** `lubancode trajectory export` 把会话账投影成训练格式，重放等级不够、沾隐私的整包剔除并注明缘由。

## [v0.26.151] - 2026-08-31

- **延迟工具不再斩断缓存前缀（需手动开启）。** 新配置 `deferred_tool_mode: proxy_reference` 下，`tool_search` 发现工具改发引用凭证，`tool_invoke` 持凭证执行——发现与调用前后请求的 system/tools 指纹逐位不变，长会话不再因翻工具列表整段重付 token。默认仍走旧路，行为一字不变。
- **派工树消耗看得见了。** 子代理逐层派工的 token 与请求数进轨迹账，配合环境快照与验证证据，`/doctor trajectory` 一站核账。（依 2026-08-31 裁决：只透明展示，不加限额。）

## [v0.26.150] - 2026-08-31

- **Token 账本接上了真账。** 每次模型调用的 system/tools 拼装现场直出 Prompt 清单（段级指纹与覆盖关系），compact、起标题、记忆抽取这些回合外小请求也进轨迹账；flag 关时零开销，行为与从前一致。
- **轨迹记账认得出环境与证据。** 开轨迹的会话自动留环境快照（git 状态、工具集、脱敏配置）；文件改动与命令执行的细账带前后指纹；改过文件后旧验证自动作废，回合结论只引新鲜证据；`/doctor trajectory` 一眼看完账本健康。
- **`search` 换引擎的地基打好。** 随包 ripgrep 的清单、定位、防 PATH 劫持与参数拼装全部就位并过单测（含真机 WSL 四幕验证），生产搜索路径一字未动，下一批接真执行。

## [v0.26.149] - 2026-08-31

- **子代理传话认得清直系。** `agent_message` 只投自己直接孩子，Dock 按层缩进画树，`/doctor agents` 能看限额、lineage 与递归开关。
- **派工不再暗自扇出没边。** `max_children_per_task`/`max_tree_nodes` 可配，超限稳定拒绝；嵌套派工的轨迹父边指向真实上级，不再冒充主会话。
- **子代理派工回到一封能干活的信。** 新调用只留扁平 `title + prompt`；双语工具提示教父代理补齐背景、已知/排除、路径与报告，宿主不再逼模型填写嵌套任务 JSON，也不拿固定信封改写正文。
## [v0.26.148] - 2026-08-31

- **`search` 工具能被取消了。** 大目录扫到一半按下取消，遍历与逐行扫描当场收手，不再空转到底才理会。
- **glob 匹配换成免回溯的状态表。** 长路径、成串星号，耗时都有确定上界，二进制文件判断也不再为一个文件开两次。
## [v0.26.147] - 2026-08-31

- **浏览器 Tab 能镜像流了。** `browser/screencast/start|stop` 起停画面直播，帧率帽默认 5fps，慢消费者队满只丢最老一帧，账追平就报，不炸内存不断连。
- **镜像帧走截图同一条落盘链路。** 不另起炉灶，`/context` 与既有截图 artifact 复用同一套内容寻址。
## [v0.26.146] - 2026-08-31

- **多渠道消息接入立下第一根桩子。** Channel 类型合同、`channel.yaml` 清单解析、Bridge v1 帧协议（握手、19 枚方法、错误码）、双向 router 骨架都到位，配一只假 sidecar 供后续真渠道适配器练手——本批不接 Telegram/QQ 这类真渠道，纯搭台子。
- **`ComponentKind` 认得 Channel 这一档。** Package 挂载事务多收一路，不必另起炉灶。
## [v0.26.145] - 2026-08-31

- **`/context` 与 `/trace` 认得出延迟工具的挂账方式。** 新增 `deferred_tool_mode` 一行，`legacy_expand` 时明示"待检索 N/M 枚"，工具执行细节里也带同一枚标签，不再是一笔糊涂账。
- **动态工具的 Prompt Cache 断口钉死一根，露了一根。** 现状回归证实：`tool_search` 命中那一拍，`system` 与 `tools` 两处指纹同时改写、同时断前缀；`break_reason()` 只报优先级最高的一根梁，复合断时另一半漏记——留了字给下一批补。
- **20 阈值这把尺子，量出两副面孔。** 轻 schema 长描述的工具，legacy_expand 首请求反倒比全常驻更贵；重 schema 短描述的工具能省七成——"延迟加载必省钱"不是恒真断言，已按实测改口。
## [v0.26.64] - 2026-08-27
