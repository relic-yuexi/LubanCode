...
// (退役,V3-LEGACY-01)原此处有"B2 gate: hub install keeps v2 bridges on
// the legacy budget path"案:env=0 建出 v2 场,验 hub.Install 只给在管预览的
// v3 桥挂 rewrite 钩子、v2 桥留在旧预算路(step-0 固定账预检与 token 校准器
// 照常)。写口退役后新建恒 v2 场造不出,该前提失效;能力位判别
// (ManagesToolResultPreviews)本身仍护恢复收养的旧盘 v2 活场,其回归随
// 旧档消费路径另立夹具单守。
