// search 内置 ripgrep 后端迁移单(P0-1)的照相机,ripgrep 迁移单 P0-8 删差分。
//
// 不进 ctest,手动跑:
...
//       记录的是"当前后端"的快照。SearchTool 走默认构造——生产定位路
//      (ExecutableDir()/libexec/rg),把本驱动 exe 放到发行包/安装位里
//       跑,就是对"包内布局真认得随包 rg"的最直接 E2E。
//   search_golden_driver bench   <输出 JSON 路径> [语料目录=src]
//       对当前后端跑一组基准查询(纯字面量/正则/无命中/高频命中/glob),
//       每档跑若干轮取 P50/P95,连同遍历文件数、耗时一并落盘。默认语料
//       是本仓 src/ 目录(medium 档);可传别的目录路径覆盖。P0-7 性能门
//       的驱动件。
//
// 历史:P0-5 曾有 diff 子命令(当前后端 vs 旧内核冻结基线 old_kernel_
// golden.json 的迁移差分门),P0-8 删——差分已过门(全等 17/批准迁移 7/
// 未批准 0,账在 docs/development/search-ripgrep-migration.md §十),旧
// 内核源码与冻结基线 JSON 都已删净,这扇门没有再开的理由;将来 rg 升版
// 若要对比,按新基线另写,不复活旧批准表。
//
// golden JSON 格式(数组,每个场景一个对象):
//   {
//     "id": "grep_chinese",                 场景名,快照比对按它对齐
//     "mode": "grep" | "glob",
//     "pattern": "...",
