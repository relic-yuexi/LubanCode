...
TEST_CASE("文件工具批余量: edit_file/search 缺省与改前一字不差") {
    ...
    // P0-5 新增的两枚批准字段。
    CHECK(ss["properties"]["fixed_strings"]["description"] ==
          "仅 mode=grep 有效:true 时 pattern 按字面量逐字匹配,不解析正则元字符;"
          "默认 false,按 Rust regex 语法解析");
    CHECK(ss["properties"]["max_results"]["description"] ==
...
TEST_CASE("文件工具批余量: en 下 edit_file/search 出英文") {
    ...
    CHECK(search.description().find("NEVER invoke rg or grep") != std::string::npos);  // NEVER 句式
    const nlohmann::json ss = search.input_schema();
    CHECK(ss["properties"]["pattern"]["description"].get<std::string>().find(
              "With mode=grep this is a Rust regex regular expression") == 0);
    CHECK(ss["properties"]["fixed_strings"]["description"].get<std::string>().find(
              "matched literally") != std::string::npos);
}
