...
void WriteSession(const std::filesystem::path& dir, const std::string& id,
                  const sessions::SessionMeta& meta, const std::vector<std::string>& lines) {
...
sessions::SessionMeta Meta(const std::string& cwd, const std::string& started_at) {
...
TEST_CASE("SessionTimeSortKey: 标准串成键,坏串折 0") {
    CHECK(sessions::SessionTimeSortKey("2026-08-20 09:30:15") == 20260820093015ULL);
    CHECK(sessions::SessionTimeSortKey("2026-08-20 09:30:15") > sessions::SessionTimeSortKey("2026-08-19 23:59:59"));
    CHECK(sessions::SessionTimeSortKey("") == 0);
    CHECK(sessions::SessionTimeSortKey("abc") == 0);
    CHECK(sessions::SessionTimeSortKey("2026-08-20 09:30") == 0);  // 位不够(只有 12 位数字)
}

TEST_CASE("SessionIdTimeSortKey: id 前缀成键,认不出折 0") {
    CHECK(sessions::SessionIdTimeSortKey("20260820-093015-试试") == 20260820093015ULL);
    CHECK(sessions::SessionIdTimeSortKey("short") == 0);
    CHECK(sessions::SessionIdTimeSortKey("2026082X-093015-a") == 0);
    CHECK(sessions::SessionIdTimeSortKey("") == 0);
}
...
TEST_CASE("SummarizeSessionContent: meta/title/cwd 事件/首句/最后 ts") {
    const std::string content = sessions::SerializeSessionMeta(Meta("D:/房", "2026-08-20 08:00:00")) + "\n" +
                                sessions::SerializeSessionMessage(UserText("第一问\n第二行"), "2026-08-20 08:00:01") +
                                "\n" +
                                sessions::SerializeSessionMessage(UserText("第二问"), "2026-08-20 08:00:02") + "\n" +
                                sessions::SerializeTitleEvent("起了标题", "2026-08-20 08:00:03") + "\n" +
                                sessions::SerializeCwdEvent("D:/新房", "2026-08-20 08:00:04") + "\n";
    const auto s = sessions::SummarizeSessionContent("20260820-080000-x", "p", content);
    CHECK(s.health == sessions::SessionHealth::Ok);
...
TEST_CASE("SummarizeSessionContent: meta 缺 started_at 退 id 时间") {
    const std::string content =
        sessions::SerializeSessionMeta(Meta("D:/房", "")) + "\n" +
        sessions::SerializeSessionMessage(UserText("问"), "2026-08-20 08:00:01") + "\n";
    const auto s = sessions::SummarizeSessionContent("20260820-093015-x", "p", content);
...
TEST_CASE("SummarizeSessionContent: queue 排队快照的 ts 也是最近的账") {
    ...
    const std::string content =
        sessions::SerializeSessionMeta(Meta("D:/房", "2026-08-20 08:00:00")) + "\n" +
        sessions::SerializeSessionMessage(UserText("第一问"), "2026-08-20 08:00:01") + "\n" +
        sessions::SerializeQueueEvent(items, "2026-08-20 08:00:30") + "\n";
    const auto s = sessions::SummarizeSessionContent("20260820-080000-x", "p", content);
...
TEST_CASE("SummarizeSessionContent: 坏 meta 单项 damaged,正文照捞") {
    const std::string content = std::string("不是 meta 的一行\n") +
                                sessions::SerializeSessionMessage(UserText("首句还在"), "2026-08-20 08:00:01") + "\n";
    const auto s = sessions::SummarizeSessionContent("20260820-080000-x", "p", content);
    CHECK(s.health == sessions::SessionHealth::Damaged);
...
struct CatalogRoom {
    ...
    CatalogRoom() {
        ...
        WriteSession(dir.base, "20260818-090000-a",
                     ...
                     {sessions::SerializeSessionMessage(UserText("甲一问"), "2026-08-18 09:00:05"),
                      sessions::SerializeTitleEvent("甲一的标题", "2026-08-18 09:00:06")});
        // 甲二:created 晚,updated 晚(同目录)。
        WriteSession(dir.base, "20260820-100000-b",
                     Meta("d:/场子甲/", "2026-08-20 10:00:00"),
                     {sessions::SerializeSessionMessage(UserText("乙段首句在这里"), "2026-08-20 10:00:05"),
                      sessions::SerializeSessionMessage(UserText("再问一句"), "2026-08-20 10:30:00")});
        // 丙一:别的目录。
        WriteSession(dir.base, "20260819-110000-c",
                     Meta("E:/场子丙", "2026-08-19 11:00:00"),
                     {sessions::SerializeSessionMessage(UserText("丙一问"), "2026-08-19 11:00:05")});
        // 坏 meta:damaged。
...
TEST_CASE("SessionCatalog: Updated 排序,新→旧") {
    CatalogRoom room;
    sessions::SessionCatalog catalog(room.dir.str());
    catalog.Scan();
    REQUIRE(catalog.size() == 4);  // 三场好的 + 一场坏,非 .jsonl 不算

    sessions::SessionQuery q;
    q.scope = sessions::SessionScope::All;
    q.sort = sessions::SessionSort::Updated;
    q.limit = 0;
    ...
    CHECK(page.entries[0].health == sessions::SessionHealth::Damaged);
...
TEST_CASE("SessionCatalog: Created 排序按 created_at,坏档退 id 时间") {
    ...
    sessions::SessionQuery q;
    q.scope = sessions::SessionScope::All;
    q.sort = sessions::SessionSort::Created;
    q.limit = 0;
    const auto page = catalog.Query(q);
    REQUIRE(page.entries.size() == 4);
...
TEST_CASE("SessionCatalog: Cwd 视图斜杠方向/大小写/尾斜杠归一后对上") {
    ...
    sessions::SessionQuery q;
    q.scope = sessions::SessionScope::Cwd;
...
TEST_CASE("SessionCatalog: 空目录/不存在的目录给空表") {
    ...
    sessions::SessionCatalog nowhere("D:/压根不存在的目录/zzz");
    nowhere.Scan();
...
TEST_CASE("SessionCatalog: 缓存指纹——文件没动不重读,动了重读") {
    ...
    WriteSession(dir.base, "20260820-090000-x", Meta("D:/房", "2026-08-20 09:00:00"),
                 {sessions::SerializeSessionMessage(UserText("旧问"), "2026-08-20 09:00:01")});
    ...
    // 动了(追加一条):updated_at 与消息数跟上。
    {
        ...
        f << sessions::SerializeSessionMessage(UserText("新问"), "2026-08-20 10:00:00") << "\n";
    }
    page = catalog.Query(q);
    REQUIRE(page.entries.size() == 1);
