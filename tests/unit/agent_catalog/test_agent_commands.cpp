...
// TUI 排版批 3:三枚 Format* 走 frame 三助手后,断言随新形状适配——信息
// 一条不删,只把旧平铺的 ": " 连排换成键值对的 "  " 两格分隔(plain 主题
// 无框,最贴近旧平铺;框/配色形状册在 tests/unit/app/test_agent_commands_frame.cpp)。
...
// 形状册口径:plain 主题(零转义、无框),断言只看内容与分隔。
const cli::Theme PlainFrameTheme() { return cli::BuiltinTheme("plain"); }
...
bool LineWith(const std::vector<std::string>& lines, std::initializer_list<std::string> parts) {
    for (const std::string& line : lines) {
        bool all = true;
...
TEST_CASE("FormatAgentCatalogListing:名称/层/可用性/描述/盖住账都进输出") {
    ...
    const std::vector<std::string> lines = app::FormatAgentCatalogListing(catalog, PlainFrameTheme());
    CHECK(Contains(lines, "Agent Catalog 共 4 个"));
    CHECK(LineWith(lines, {"general-purpose", "builtin", "可用"}));
    CHECK(LineWith(lines, {"Explore", "builtin", "可用"}));
    CHECK(LineWith(lines, {"layered", "user", "可用"}));
    CHECK(Contains(lines, "用户层定义"));
    CHECK(LineWith(lines, {"broken", "user", "不可用:"}));
...
TEST_CASE("FormatAgentDoctorReport:查无此名给一行指路") {
    ...
    const std::vector<std::string> lines =
        app::FormatAgentDoctorReport(catalog, "no-such", {}, {}, PlainFrameTheme());
...
TEST_CASE("FormatAgentDoctorReport:坏定义逐条摆诊断,结论不可用") {
    ...
    const agent::AgentCatalog catalog = fx.Load();
    const std::vector<std::string> lines =
        app::FormatAgentDoctorReport(catalog, "broken", {}, {}, PlainFrameTheme());
    CHECK(LineWith(lines, {"定义", "不可用"}));
    CHECK(Contains(lines, "[错误]"));
    CHECK(Contains(lines, "未知字段"));
    CHECK(LineWith(lines, {"结论", "不可用"}));
}
...
mcp_servers:
  - browser
    ...
    const std::vector<std::string> lines =
        app::FormatAgentDoctorReport(catalog, "probe", materials, {}, PlainFrameTheme());
    CHECK(LineWith(lines, {"定义", "解析通过"}));
    ...
    CHECK(LineWith(lines, {"runtime", "max_output_tokens=继承"}));
    CHECK(Contains(lines, "max_steps_per_turn=继承"));
    CHECK(Contains(lines, "length_continuations=继承"));
    CHECK(LineWith(lines, {"permissions", "inherit"}));
    CHECK(LineWith(lines, {"结论", "定义可用,但静态预检发现"}));
}
...
TEST_CASE("FormatAgentDoctorReport:材料全空时跳过比对,不猜") {
    ...
    const agent::AgentCatalog catalog = fx.Load();
    const std::vector<std::string> lines =
        app::FormatAgentDoctorReport(catalog, "plain", {}, {}, PlainFrameTheme());
    CHECK(LineWith(lines, {"定义", "解析通过"}));
    CHECK(LineWith(lines, {"工具引用", "会话工具表不可用,跳过比对"}));
    CHECK(LineWith(lines, {"Skill 预装", "无"}));
    CHECK(LineWith(lines, {"MCP", "无"}));
    CHECK(LineWith(lines, {"结论", "静态预检通过"}));
}
...
TEST_CASE("FormatAgentDoctorReport:覆盖链摆出被盖住的来源") {
    ...
    const agent::AgentCatalog catalog = fx.Load();
    const std::vector<std::string> lines =
        app::FormatAgentDoctorReport(catalog, "general-purpose", {}, {}, PlainFrameTheme());
    CHECK(LineWith(lines, {"来源", "user "}));
...
TEST_CASE("P1-0 doctor:预算合同列明生效路,legacy 定义给迁移建议") {
    ...
    const std::vector<std::string> legacy_lines =
        app::FormatAgentDoctorReport(catalog, "legacy", {}, {}, PlainFrameTheme());
    CHECK(Contains(legacy_lines, "[警告]"));
    CHECK(Contains(legacy_lines, "agent.legacy_step_budget"));
    CHECK(LineWith(legacy_lines, {"预算合同", "legacy per-run step 预算 9"}));
    CHECK(LineWith(legacy_lines, {"迁移建议", "删掉 runtime.max_steps_per_turn"}));
    CHECK(Contains(legacy_lines, "runtime.max_turns: 9"));

    // 新字段定义:任务 turn 预算一行,归属行,不给迁移建议。
    const std::vector<std::string> fresh_lines =
        app::FormatAgentDoctorReport(catalog, "fresh", {}, {}, PlainFrameTheme());
    CHECK(LineWith(fresh_lines, {"预算合同", "task turn 预算 12(来源: Agent Definition runtime.max_turns"}));
    CHECK(LineWith(fresh_lines, {"预算归属", "TaskLedger 任务记录"}));
    CHECK_FALSE(Contains(fresh_lines, "迁移建议"));

    // 两枚都没写:如实说落宿主默认。
    const std::vector<std::string> bare_lines =
        app::FormatAgentDoctorReport(catalog, "general-purpose", {}, {}, PlainFrameTheme());
    CHECK(LineWith(bare_lines, {"预算合同", "未显式声明"}));
}
...
TEST_CASE("P1-0 inspect:legacy 定义给可复制的迁移片段,新字段不给") {
    ...
    const std::vector<std::string> legacy_lines =
        app::FormatAgentInspectReport(catalog, "legacy", {}, PlainFrameTheme());
    ...
    const std::vector<std::string> fresh_lines =
        app::FormatAgentInspectReport(catalog, "fresh", {}, PlainFrameTheme());
...
TEST_CASE("doctor 的 Profile 行:覆盖存在报层数,不存在打 ✗,default 不查") {
    ...
    const std::vector<std::string> named =
        app::FormatAgentDoctorReport(catalog, "named", {}, ctx, PlainFrameTheme());
    CHECK(LineWith(named, {"Profile", "browser-tester(三层共"}));
    CHECK(Contains(named, "个模块覆盖;/agent inspect named"));

    // no-such:三层全空,打 ✗ 并指路建目录。
    const std::vector<std::string> ghost =
        app::FormatAgentDoctorReport(catalog, "ghost", {}, ctx, PlainFrameTheme());
    CHECK(LineWith(ghost, {"Profile", "no-such ✗"}));
    CHECK(Contains(ghost, "profiles/no-such/"));

    // 没点名 Profile:default 上下文,不进 ✗ 账。
    const std::vector<std::string> plain =
        app::FormatAgentDoctorReport(catalog, "plain", {}, ctx, PlainFrameTheme());
    CHECK(LineWith(plain, {"Profile", "继承(落回 default)(default 上下文,三层覆盖不参与)"}));
...
TEST_CASE("FormatAgentInspectReport:定义来源 + prompt 三笔 + 逐模块来源账") {
    ...
    ctx.user_prompts_dir = prompts.Str();
    const std::vector<std::string> lines =
        app::FormatAgentInspectReport(catalog, "probe", ctx, PlainFrameTheme());
    CHECK(Contains(lines, "agent inspect: probe"));
    CHECK(LineWith(lines, {"定义来源", "user "}));
    CHECK(LineWith(lines, {"prompt", "profile=browser-tester · project_instructions=omit · soul=off"}));
    ...
    CHECK(LineWith(lines, {"依赖预检(Skill/MCP/工具/模型)", "/agent doctor probe"}));
}
...
TEST_CASE("FormatAgentInspectReport:default 上下文与查无此名") {
    ...
    const std::vector<std::string> missing =
        app::FormatAgentInspectReport(catalog, "no-such", {}, PlainFrameTheme());
    REQUIRE(missing.size() == 1);
    CHECK(missing[0].find("没有叫 \"no-such\"") != std::string::npos);

    const std::vector<std::string> lines =
        app::FormatAgentInspectReport(catalog, "plain", {}, PlainFrameTheme());
    CHECK(LineWith(lines, {"prompt", "profile=继承(落回 default) · project_instructions=inherit · soul=inherit"}));
