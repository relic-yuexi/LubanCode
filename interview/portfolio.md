...
[文档首页](../docs/README.md) · [面试深挖导航](deep-dives.md) · [开发难题与故障复盘](retrospectives/development-challenges.md) · [架构说明](../docs/architecture/README.md) · [Query 数据流](../docs/architecture/query-data-flow.md)

这页管项目介绍、简历文案、面试故事与演示。面试官追问模型目录、JSON Schema、Agent Loop、重试恢复、context compact、长文件、命令、tool use、MCP、Skill、Hook 与记忆实现时，转到[面试深挖导航](deep-dives.md)，再沿专题页追源码。

> 本页是演示与求职素材，不是产品事实的权威来源。本次未刷新测试数、提交数与代码行数，正文不固定这些数字。使用前须按“数据刷新”一节重算，并标日期与 commit。产品行为请回[文档首页](../docs/README.md)查对应专题。
...
| 产品代码 | 发布前按“数据刷新”重算，并标日期与 commit |
...
- 建立 CMake + doctest + GitHub Actions 交付链；CI 在 MSVC/GCC/Clang 三路编译测试，并按标签生成 Windows/Linux/macOS 发行包。简历若要报测试数，须先按“数据刷新”复测。
...
- [`docs/extensions.md`](../docs/features/extensions/README.md)
...
- 块盒只存 `rows/width/baseline`。分式线充当 baseline，横排按最大 ascent/descent 对齐；多行括号用 Unicode 部件逐行拼，不冒充完整 TeX 字体度量。
...
当前 BM25 用 `k1=1.5`、`b=0.75`，是常用起点，尚未按真实记忆卡长度扫参。
它检索的是结构化 `MemoryEntry`，不是整份源码；固定尺子量过最终召回，没有做
`k1/b` 消融。分层 compact 的 reduce 最多两两归并四轮；四轮后仍超窗时还会发
终稿请求，等 provider 报错，旧 history 虽不动，本地收口仍欠一闸。
...
砍块级 LaTeX。它能显出 parser、基线与盒式布局功夫，却不在 coding agent 的前三件
事里。首版让公式原样显示，把工时还给 session 恢复、排队 ack、默认终端 CI 和真实
使用。三协议与 compact 还守着项目主线；公式排版没有。

### 为什么不直接改 OpenCode / Codex？

只求一只好用工具，直接用现成项目。自建是为了亲手做 C++ agent runtime、原生终端、
跨平台进程与 provider-neutral loop。这能说明学习动机，不能证明商业总成本更低。
Git 只记载早期模型目录借鉴 Codex model-catalog；OpenCode 是后来作的同类对照。

许可证也不能含糊：根目录眼下没有 `LICENSE`，README 的 Apache-2.0 链接是断的，第三方
归属账也没建。故答“来源写过，许可证闭环没做完；发版前须审上游 commit、条款与
改写范围，再补项目许可和第三方声明”。

### 最大遗憾是什么？

太早把它当作品集，太晚把它当日用工具。我喜欢往上加能演示的功能，却没有先拿连续
dogfooding、coverage、durable ack、默认终端 CI 与许可证把地基压实。若重来，核心
闭环一成便冻功能，先让自己和几位外部用户做真任务；没人反复需要的功能，不进主线。

### 单文件分发兑现成什么数字？

2026-08-30 本机 `0.26.127` Windows x64 Release 为 `11,575,296` 字节，
即 `11.04 MiB`。程序主体是一只 `lubancode.exe`；发行包另带 skills、文档与安装脚本。
仓根眼下缺 `LICENSE`，故不能再说“许可证已随包带上”。
...
### Plugins 有什么用？为什么不全用 MCP？

有些工具本来只是一枚可信本地函数。若为它另起 MCP server，还要备运行时、拉子进程、握手、走 JSON-RPC、管超时与退出。Lua 插件用一份脚本承接轻逻辑；C ABI 插件接原生库与系统能力。二者仍适配成统一 Tool，照走 Schema、确认、Hook 与回填。

MCP 也没有多余。它用进程与协议成本换隔离、语言自由、独立状态和生命周期。不可信扩展、已有服务、浏览器或数据库连接，仍该走 MCP。插件与 MCP 不是替代关系，是函数边界与服务边界的分工。

安装也能当场演示：复制 `examples/plugins/word_count.lua` 到用户插件目录，重启后用 `/plugins` 查；Windows 还可把 `hello_plugin.c` 编成 DLL 真加载。更深的 ABI、并发与安全边界见[进程内插件系统深挖](../docs/architecture/extensions/plugin-runtime.md)。
...
5. 排队消息已有 session 快照、恢复与失败回队，却还没有 `pending / inflight / acked`
   的耐久收据。队列清账与目标 history 之间仍有双写裂口。
...
| “测试覆盖率很高” | “在 `<commit>` 的 `<环境>` 跑完全套，结果为 `<通过/失败/跳过>`”；没有 coverage 数据便不报百分比 |
...
正文、工具状态和可编辑输入并发写屏。CI 覆盖 MSVC、GCC 与 Clang；工程
规模和本地测试结果按投递时 commit 复测后填写。
